...
- **INV-6 A change is retained only against a measurement.** A change claimed
  ...
  **26491479 from S207 on 2026-09-11**, which moved the bench total without
  moving one `search_bench` count, for the reason the search row gives, and
  **30046849 from S208 the same day**, which moved it again for a different
  reason: the bench set's one illegal position became legal (DEC-177), so the
  eighth tree is a different tree. `search_bench` did not move at either step.
...
| INV-6 | `tools/gate.sh` for the neutral half -- the `bench` signature against the commit message's `Bench:` line, `test_gate_script` over its nine decisions and `test_uci_surface` *"bench prints one final signature line and repeats its total"* over the command; `tools/search_bench.py` still gives the per-position counts and best moves; an SPRT against a named commit for the half that alters play. **And the sanitizer build's `bench` total equals the Release one**, asserted in `tools/gate_extra.sh` stage 4 (S197, DEC-167): the two directories differ only in instrumentation, so a difference is a read of uninitialised or out-of-bounds memory that moved the tree and that neither sanitizer reported. It is the only reading of INV-6 that compares two builds of one commit rather than two commits |
...
**The `info` line's field set is golden since 2026-09-13, S213 (DEC-184).**
`MANUAL.md`'s "What a search prints" is normative, and `tests/test_uci_surface.cpp`
holds `expected_info_fields` against three ends at once -- the golden list, one
`go depth 6` on the start position, and the manual's own example -- so a field
added to or dropped from the engine's `info` line, or from the manual, goes red
from both directions (observed with `hashfull` spliced into the engine and
`seldepth` into the manual). The `test` command's table lost its three stale
labels rather than re-deriving them; the site says why.
...
| position input | `load_FEN` validates syntax **and the two semantic classes that corrupt** since 2026-08-22, S161. A castling right whose king or rook is not on its square, and an en-passant square that is on the wrong rank for the side to move, has no enemy pawn on the victim square, has an occupied target square, **or -- since 2026-09-12, S042 -- has no pawn of the side to move standing where it could play the capture (the X-FEN reading, pseudo-legal, pins not examined)**, are **cleared and the position is accepted** -- not rejected, because GUIs, books and conversion tools send stale fields and the match harnesses sanitize rather than refuse. Both clearings happen after the fields are parsed and before `compute_full_hash()`, so a sanitized position hashes as the position it actually is. This is prime-directive work and not a strength item: the generator emits castles from the hard-coded `e1`/`e8`, `castling_rook()` answers from the king's target square rather than from the board, and every piece update is an xor -- so an unsupportable field did not produce an illegal move, it **created pieces**, silently in the Release build that ships and is measured. Perft (INV-1) could never see it, every perft FEN being semantically valid. Behaviour-neutral on every valid FEN and discharged on node counts: 121512 / 800769 / 62907 and `c3d5` / `e2a6` / `d7c8q` at depth 9, identical to `0a274c9` (INV-6). `2026-08-22_adversarial-F01`. **Two further classes are refused rather than repaired since 2026-09-11, S208**, and they are refused because neither can be repaired without inventing a position: **more than 16 pieces of one colour**, and **a pawn on rank 1 or rank 8**. Each names its own reason on the UCI channel in S176's shape, and `set_position()` already restores the whole game on a failure, so the board, the history and every move applied since the last `position` are left as they were. What they cost before: `QQQQQQQQ/k6Q/Q6Q/Q6Q/Q6Q/Q6Q/Q6Q/KQQQQQQQ w - - 0 1` loaded cleanly, generated **277** moves into the `move_t[270]` every caller declares on the stack, and aborted the **Release** binary with `*** stack smashing detected ***`, exit **134**, core dumped (`2026-09-10_adversarial-F09`); `P7/8/8/8/8/8/8/K6k w - - 0 1` and `K6k/8/8/8/8/8/8/p7 b - - 0 1` made `evaluate_pawns()` read `passed_pawn_mg[6]` off a six-entry table, which the sanitizer build reports as `index 6 out of bounds for type 'int [6]'` at `src/evaluation.cpp:516` and `:529`, and which in Release writes one `int` past the collecting caller's `int[2][6]` (`-F10`). Neither is reachable from legal play and no GUI sends either; both were reachable from `position fen`, which is also how corpora, `tools/pgn_to_positions`, `datagen` and any fuzzer reach the engine. **Two more are refused rather than repaired since 2026-09-13, S223**, and they close the rest of the sentence S161 wrote: **other than one king of each colour**, and **the side not to move in check** -- the engine's own attack test run from that king's square, which makes two adjacent kings a refusal by the same test rather than by a rule of their own. Same S176 shape and the same whole-game restore as S208's two, and they run after them, so a placement that breaks both hears S208's reason. What they cost before: `position fen 7k/8/8/8/8/8/8/K6R w - - 0 1 moves h1h8` was accepted and left `7R/8/8/8/8/8/8/K7 b - - 0 1`, the black king captured by a move the engine played (`2026-09-12_adversarial-F01`); `8/3p4/8/8/8/8/3P4/8 w - - 0 1` and `4K3/8/8/8/8/8/8/K6k w - - 0 1` loaded and were echoed back by the `fen` command. The refusal agrees with python-chess 1.11.2 `Board.status()` -- `NO_WHITE_KING`, `NO_BLACK_KING`, `TOO_MANY_KINGS`, `OPPOSITE_CHECK` -- over every FEN literal in `tests/`, `src/` and `tools/`: **3214 unique, 0 disagreements** over the 3199 that S208's classes and the parser do not answer for first (`adocs/data/S223_fen_census.py`, output in `S223_fen_census_after.txt`). Four branches that existed only for a kingless board became Debug assertions -- `is_check()`, `generate_moves_impl()` and `king_shelter_features()` in S223, `king_zone()` in S213 the same day -- and `make_move()` gained one that each side still has exactly one king after the move; `EMPTY_POS` and the `position empty` shortcut are gone, a shortcut for a board no loader accepts having no use. Node-identical, so the commit says `No functional change`: `bench` **7105111** and `search_bench` **47635 / 213916 / 26130** at depth 9 and **141455 / 1038779 / 175684** at depth 12, all unmoved, with the same best moves (INV-6). DEC-197. **Legality at large is still deliberately unchecked beyond those four** -- pawn counts below the 16-a-side bound, promoted-piece plausibility, the number of pieces giving check (python-chess's `TOO_MANY_CHECKERS` and `IMPOSSIBLE_CHECK`) and reachability at large are not tested, and the engine plays all of them correctly. **The 16-a-side bound cost one bench position its illegality**: `KILLER_POS` carried 17 white pieces and was kept on the grounds that the engine loaded it, which S208 made false, so its h3 pawn is gone -- `Status.VALID` to python-chess, the twelve promotions and the `f5e6` en-passant capture intact, legal moves 42 -> 48 -- and **`bench` moved 26491479 -> 30046849** while `search_bench` stayed node-identical at `121530 / 801481 / 72924`, which is the half of INV-6 that speaks for legal positions. DEC-177 |
...
| search | alpha-beta, transposition table, quiescence, PVS, aspiration windows, null move pruning, late move reduction, reverse futility pruning, staged generation, killers, history, countermoves, insufficient-material draws. **Quiescence scores them too since 2026-09-13, S210**: `is_insufficient_material()` had one call site, `negamax_at` at `ply > 0`, so a capture inside quiescence that left KvK, KNvK or KBvK was handed to `evaluate()` and scored on the material left standing (65, 361 and 407 on the case's own three positions). The test is made on the position a move leaves rather than on the node entered -- the same set, since material is all the function reads and the entered node was answered a ply higher -- so the child is never created; unconditional, not gated on a capture, because a quiet promotion changes material and quiescence searches quiet moves in check. Reach counted before any match was booked (`adocs/data/S210_f22_census.py`, 11503 positions of the S219 A/A at depth 10): 0.062 % of the moves made in quiescence land on a dead board and **194 of 11503 root answers move, 1.687 %**, so DEC-107's census discharge does not apply and one `--nonreg` `{-5, 0}` run was owed and taken (`adocs/data/S210_f22_sprt.sh`, `9ef06f3` against `9075bf8`, 2026-09-13 to 14): **H1, LLR 2.98, `Elo 0.01 +/- 3.17`, `nElo 0.02 +/- 4.03` over 28598 games, 0 forfeits, and 0 of the 1661 games adjudicated drawn by insufficient material carry a mate score** -- not a regression, the point estimate at zero. `bench` 7111579 -> 7105111; `search_bench.py` node-identical at depths 9 and 12, which says only that its three positions never reach the class (2026-09-10_adversarial-F22). **One-ply continuation history was added and reverted on 2026-09-12, S024 verdict 1 (DEC-194)** -- built as follows, measured **H0** at `{0, 5}` nElo (LLR -2.96, nElo -5.48 +/- 7.20 over 8954 games, 0 forfeits) with the table exercised in 96 % of quiet reads, and taken out of the tree the same day; it returns as S222 with its own fitted scale. What was built: a table indexed by the previous move's piece and target and the current move's piece and target, written where plain history is written (the cutoff move credited, the quiets tried before it charged, the same graded formula) and read in `score_move` summed with plain history; every entry is gravity-bounded by `QuietHistoryMax` like plain history, so the quiet band's width doubles and still clears the countermove band above it, asserted in `tests/test_evaluation.cpp`; the table is indexed through one helper, `continuation_entry`, and only when a previous move exists -- ply 0 and the node after a null move pass none and touch nothing, asserted in `tests/test_search.cpp` with the guard's mutant killed. No constant of its own; the shared constants are fitted in S127's lane. `bench` 27322394 -> 22363740 while it was in, 27322394 again after the revert. **A repetition is scored as a draw only when the search itself walked into it, or when the position has occurred twice before, since 2026-09-11, S207**: `classify_repetition()` compares the index of the matching history entry against `search_state_t::root_history_size` -- `history.size` at the moment `search()` was entered, so the entry at that index is the root position itself -- and returns `DRAW` for a match strictly above it, or for a second match wherever both lie; `negamax_at` scores `DRAW_SCORE` on that class alone. **The key the compare reads is right since 2026-09-12, S042 (DEC-187)**: `make_move` used to xor the en passant key on every double push whether or not a pawn could capture, so the position after the push hashed differently from the identical position reached another way and `classify_repetition` skipped that occurrence -- 236 of 1500 self-play games at S219's comparison, 52 published scores wrong, -313 against 0 for one position and history. One helper, `en_passant_is_capturable`, mirrors the generator's candidate test and decides the square in `make_move_impl` and `load_FEN`, so the incremental key equals `compute_full_hash()` at every position and `generate_FEN`'s fourth field agrees with python-chess's X-FEN on all 115021 positions of the S219 A/A corpus. `bench` 30046849 -> 27322394; `--nonreg` `{-5, 0}` against `a3e84e1` on `noob_3moves.epd`: **H1, LLR 2.95, nElo +9.93 +/- 8.99 over 5741 games in 2 h 40 m 45 s, 0 forfeits**, not a regression of 5 nElo and no gain claimed. fastchess's "PV continues after threefold repetition" fell from 427 on the reference side to 1 on the candidate over the same games; the one is a PV-instability artefact on a threefold that follows a queen move, not a key defect. The published rule, and the reason it is not symmetric: a draw is returned if the position repeats once earlier but strictly after the root, or repeats twice before or at it, because a side that repeated inside the tree can repeat again behind the horizon while the side to move at the root gets to choose again and the opponent cannot force the third occurrence alone (CPW *Repetitions*; Stockfish PR #925, read as prose -- no source was opened, DEC-016). **The convention it replaces scored any earlier occurrence in the halfmove window as a draw, and no decision in this repository ever recorded it.** What it cost, measured at `ae4eed4` and not carried over from the audit: the start position less Black's queen with the history `g1f3 g8f6 f3g1`, `go depth 10` driven through python-chess's `chess.engine`, answered `score cp 0, nodes 5387, pv f6g8` against `score cp -1229, nodes 412680, pv e7e5` for the identical board with no history -- a false draw behind a tree **77 times smaller**, published for a position the engine is losing by a queen. Oracles, neither of them chesso: python-chess 1.11.2 on the position after `f6g8` reports `is_repetition(2) True`, `is_repetition(3) False`, `can_claim_threefold_repetition() False`, `is_game_over(claim_draw=True) False`, and stockfish at depth 18 with the same board and move history scores it **-703**. No draw exists. **It fires in ordinary play**, which is what separated it from the other findings of the 2026-09-10 audit: fastchess sends every game as `position startpos moves ...`, so the pre-root history *is* the game, and `tools/analyse_game.py` -- the tool `CLAUDE.md` mandates because agent chess judgement is banned -- reads those scores. After the change both readings are `-1229 / 412680 / e7e5`, identical with the history and without it. **`bench` moves 26851183 -> 26491479** (-1.34 %) while `search_bench` at depth 9 is node-identical -- `121530 / 801481 / 72924`, `c3d5` / `e2a6` / `d7c8q` -- and the two together say something exact rather than contradicting each other: the only class reachable from a bare FEN is the root position recurring once inside the tree, which the old rule called a draw and this one does not, and none of the three `search_bench` trees returns to its own root. INV-6 is therefore not available here and the games decided it: **`--nonreg` `{-5, 0}` nElo, alpha = beta = 0.05, H1 accepted -- LLR 2.96, Elo +3.32 +/- 5.00, nElo +4.47 +/- 6.72, LOS 90.36 %, PairsRatio 1.05, Ptnml [434, 1025, 2135, 1079, 456] over 10258 games in 4 h 26 m 56 s, 0 time forfeits of 10259 on either side** (`adocs/data/S207_sprt.sh` is the pre-registration, written before the first game). **What H1 at that pair establishes is that this is not a regression of 5 nElo or more, and nothing else**: an SPRT stops early exactly when the observed effect has run favourable, so the +3.32 is biased upward and is not the effect size (DEC-063). No gain is claimed; what the change is for is the score, and the oracles above are what establish that. Two fast-suite cases pinned the old convention and were **re-stated, not relaxed** (DEC-173) -- "the losing side takes an available repetition" and "a repetition is answered before the table is", each now reaching its position a third time, so the property each asserts is unchanged and the first's old input became the new red-first case. The two new cases are "one occurrence before the root is not a draw" and "the root's own occurrence is the boundary", the second being what separates `>` from `>=`; they kill mutants M35 and M36. **The boundary itself -- `search()`'s `state->root_history_size = game->history.size` -- is pinned since 2026-09-11, S215**, by a third case, "search() hands the rule the root's own index": one board searched three times through `search()` rather than through `negamax()` with the field set by hand -- a forced perpetual at depth 4 whose one pre-root entry is **not** the root position, where the cycle returns to the root's own entry and that is not a draw; the same history at depth 5, where the ply-1 position returns strictly inside the tree and is; and the same board with the cycle played before the root, which is a draw at the depth that answered the material. Every wrong boundary was observed red: `- 1` and `0` on the first, `+ 1` and the deleted assignment on the second. Until then nothing in the suite read that line -- mutated to `history.size - 1` the whole fast suite stayed green while `bench` moved **13 %**, 30046849 to 26117924. **What moves it is the underflow and not the root-recurrence class**, measured by S215's own fast check and then by an A/B: every `bench` root is loaded from a FEN and `load_FEN` zeroes the history, so `size - 1` is `SIZE_MAX` there and **no** in-tree draw is ever detected -- an explicit `SIZE_MAX` benches **26117924**, M39's total to the node. The root-recurrence reading is what `- 1` does to a root with a history behind it, which is ordinary play and is what the case asserts. Mutants M39 and M40. 2026-09-10_adversarial-F08. **A node whose halfmove clock has reached 100 scores a draw only once checkmate has been ruled out since 2026-08-22, S162**: FIDE 5.1.1 ends the game the instant checkmate is delivered and 9.6.2 states the same exception for the 75-move rule in so many words, so the clock test runs `is_check()` and one move generation before returning `DRAW_SCORE`, and a mated node falls through to the two sites that already know the distance -- `negamax`'s own no-legal-move return and quiescence's -- rather than computing a second copy of that arithmetic. Neither call is paid at a node whose clock has not reached 100, which is every node any benchmark visits. What it cost before: on `7k/6pp/8/8/8/7n/6P1/R6K w - - 99 60` at depth 1, where the clock reaches 100 on the mating move, the engine reported `cp 448` and played `g2h3`, discarding a mate in one it had already generated -- stockfish scores the same position `Mate(+1)` and python-chess reports the child mate at a clock of exactly 100. **The boundary itself is pinned since 2026-09-09, S193**: a root at clock 99 -- 21 replies, every one quiet, all landing on clock exactly 100, none of them mate and none insufficient material, established from the engine's own generator -- scores exactly **0** at depth 1, and the same placement at clock 98 does not (**929**). The clock-100 case cannot see the boundary: the root is exempt, so every node it reaches is at 101 or more, and an off-by-one to `>= 101` survived **all 27 test binaries** as M19 of the 2026-09-04 test review. The insufficient-material test immediately below cannot intercept a mated node: over all **1700560** legal positions it calls a draw -- two kings with at most one knight or bishop, either side to move -- exactly **0** are checkmate, enumerated rather than argued. Bench node counts and best moves are identical to `ecd735e` -- 639228 / 3430710 / 367858 at depth 12, `c3d5` / `e2a6` / `d7c8q` -- but that is not the whole of INV-6 here, because it only says no bench FEN approaches the boundary. **What carries it is a census over the games, and the insurance SPRT was killed on it (DEC-107)**: over 3356 games at 8+0.08, 102 peaked at a halfmove clock of 80 or more, so the branch does execute in about 3 % of games and 55 reached clock 100 on the board -- and the positions that were checkmate at a clock of 100 or more numbered **0**, at 90 or more also 0. The run had reached `LLR 0.09` in 3304 games, 2.9 % of the way to a bound, so its only reachable ending was the null its own accepts had pre-declared. `adocs/data/S162_clock_census.py` is the count and `adocs/plan_done/S162_fifty_move_mate_precedence.md` is the ledger. Reverse futility returns a static lower bound instead of searching and therefore cannot see a mate -- **and since S235 (landed 2026-09-25) what it returns is a point between beta and that bound**, `RfpReturnWeight` hundredths of the way up from the first to the second, rounded toward beta -- **since S234 (landed 2026-09-24) the number it is decided on is the node's estimate and not the static score, at `RfpTtEstimate` 1**: the transposition entry's own score where the entry's bound type certifies the direction that score has moved in from the static evaluation (a lower bound above it, an upper bound below it), never a score inside the mate band, never at a node in check and never from an exact entry -- S109's own rule with a second consumer rather than a new one -- and the bound the rule argues is that same estimate less the margin, which on the lower-bound branch is weaker than what the entry already certifies; **what the node hands its parent is `RfpReturnWeight` hundredths of the way from beta up to that bound** (S235), which at 100 is the bound itself and is the tree before that step, bench signature included, and at 0 is beta; the weight moves what a pruned node returns and never which nodes prune, it is independent of `RfpTtEstimate` because it blends whichever number the margin was subtracted from, and both ends are strictly inside the mate band, so no setting of it can return a mate score. Decided by one `{0, 5}` nElo SPRT against the tree without it (`adocs/data/S235_sprt.sh`): <verdict>; `RfpTtEstimate` 0 puts both the comparison and the bound back on the raw static evaluation and is the tree before the step, bench signature included (DEC-215). The estimate is a pruning input and never a stored one: the entry's evaluation field and the per-ply static scores the improving flag compares both hold what `evaluate()` returned at every setting, and no history update reads a static evaluation at all. This engine's null-move block has no static-score condition and razoring does not exist until S116, so reverse futility is the whole of the routing. Decided by one `{0, 5}` nElo SPRT against the tree without it (`adocs/data/S234_sprt.sh`): **H1 on 2026-09-25, `Elo 4.73 +/- 3.74`, `nElo 6.09 +/- 4.81`, 20062 games in 9 h 32 m, 0 forfeits either side** (`adocs/data/S234_sprt.log`), so the switch ships at 1 and stays for S116's second site, and S095's guarded mate row ships re-mined on this tree at depth 8 (DEC-142); it is **skipped at plies 0, 1 and 2 and applies from ply 3 down, with a remaining depth of 15 or less**, and five guards that were meant to make it mate-safe measured red or inert (2026-08-16, S033). Both bounds were SPSA-tuned in S085, the depth one from 6, and at 15 the assumption is made at every depth this engine reaches -- median 11 at the tuning control -- so the depth bound has stopped being a guard and `RFP_MIN_PLY` is the one that carries it. **S145 measured what that costs and what the floor is worth**, on a constructed set of forced mates spanning mate distances two to five so the guarded defender nodes land at plies 1, 3, 5 and 7, each proved by exhaustive AND/OR enumeration and corroborated by stockfish, neither of them chesso (`adocs/data/S145_mate_set.py`) -- **48 of them when S145 built it and 82 since S168 on 2026-09-01**. **That set was one motif when S145 built it, is three since S168, and the shape is counted rather than conceded (2026-09-01, S155 and S168)**: `adocs/data/S155_motif_census.py` reads the tracked TSV and reported **two material signatures over the 48, each the colour mirror of the other**, with a **lone queen as the mating force in 48 of 48** and `lead` **760 in 48 of 48**. DEC-114 is the owner's decision that one mating piece across a gate built to catch mating-piece defects is not enough. Over the 82 rows the census reports **five material signatures and three mating forces -- a queen in 48, a lone rook in 32, two knights in 2 -- with leads 760, 1160 and 1020**; S168 added families and replaced none, so every position S145 proved is still in the file. The narrowness that remains is close to forced by the hazard and not a defect in the set: the rule misfires only where the side to move is lost by force while materially ahead, and a frozen clump behind a blocked wall is close to the only way to build it. **Two things S168 measured are worth more than the rows it added.** A king and two knights does *not* give a knight mate by construction -- a mobile knight standing beside a wall pawn unfreezes the capture the non-adjacent files deny and the freed pawn queens with check, which is how **twelve of the first fourteen knight positions came out mated by a pawn**; the generator enforces the mating piece now and refuses a candidate whose line offers a promotion anywhere, under which three of the four knight families accept nothing and the fourth accepts two, so an all-quiet knight mate is **rare** and the count says so rather than hiding it. And the rook families are named for their force because the geometry did not survive being checked: the mate lands on the mated side's own back rank in **13 of 32**, so DEC-114's "back-rank mate" is in fact a lone-rook mate that is sometimes one. What the gate still cannot catch is stated as a list and not implied -- **no knight mate deeper than a mate in two (both knight rows are mates in two), no smothered mate, no king hunt, no open-line mate or line-opening sacrifice, no promotion mate -- the engine's own generator emits **1981 legal moves over the 82 roots and the 186 guarded defender nodes, of which 2 are pawn moves and 0 are promotions** (`S155_motif_census.py --moves`), and no position with a realistic material balance**; a pruning rule that hides a mate in any of those shapes passes this suite. **The breadth half of the gate is asserted since 2026-09-01, S156**, and it is the opposite instrument: `adocs/data/S145_mined_set.tsv` is 318 positions taken one per game from the engine's own SPSA games and labelled by stockfish at a node limit, nothing in it was built for the hazard, and `test_mate_breadth` scores the whole set at depth 10 as **a count with a floor of 143 exact plus zero wrong signs** -- never per position, because a search is allowed to miss any particular deep mate and a suite that forbids it is a suite that gets switched off, which is what happened to two of the seventeen engines S145 surveyed. Re-measured at that step's commit: **145 exact and 147 right-signed at `RfpMinPly` 3 and 2, 141 and 143 at 1 and 0**, so the floor sits strictly between what ships and what the weakened guard gives; S145 read 146 against 139 and the gap has narrowed from 7 to 4 as S142, S149 and S165 moved the tree. Reaching 1 and 0 needs the bound relaxed in a throwaway worktree, S142 having made 2 the declared minimum -- `adocs/data/S156_mined_floor_sweep.py` is that sweep and it also rebuilds the gate with the weakened value as its compiled-in default to observe the red. The reading is almost entirely a function of the mate distance: at the shipping defaults over the 82 the engine reports **26 of 26 mates in two with no delay, 12 of 24 mates in three, 1 of 16 at four and 0 of 16 at five** (S168, 2026-09-01; over S145's 48 the same sweep read 16/16, 9/16, 0/8 and 0/8), and no setting of either bound produces a false mate -- 0 wrong signs and 0 distances shorter than the proved minimum, over six settings times 82 positions and before that twelve times 48. The floor separates cleanly and the enlarged set separates it wider: `RfpMinPly` 2 and 3 are identical at 12 of 24 mates in three with every mate in two immediate, while 1 and 0 -- the same engine, the root being exempted by `!is_pv` -- drop to 10 of 24 and, on the mates in two, to 21 of 26 found and **15 of 26 on time**, eleven positions failing the clause that fences the tuner where seven failed it over the 48. So `MATE_IN_THREE_FLOOR` is **11**, strictly between the 12 that ships and the 10 the removed guard gives, re-derived because S168 moved both ends and DEC-116 requires re-derivation whenever either moves. The deep classes are recovered by the *ceiling* and not by the floor, and **S148 measured what recovering them costs and decided against it, 2026-09-09, DEC-158**. Over the 82-row set at every ceiling from 0 to 15, the mate in five class is a cliff -- 11, 9, 6, 4, 1 of 16 at ceilings 0 to 4 and 0 from 5 up -- and the whole set reads 39 of 82 exact at the shipping 15 against 52 at 4, the largest ceiling where both deep classes survive. That candidate lost its SPRT: **nElo -7.31 +/- 5.60, Elo -5.66 +/- 4.33 over 14808 games at 8+0.08, H0 accepted against {-5, 0}**. So the ceiling stays at S085's 15 and the mates it loses -- **1 of 16 in four, 0 of 16 in five**, against 7 and 1 at the ceiling that was refused -- are its measured price and not an open question. The plateau starts at 10, not at 15: every count from 10 up is identical, so the shipping value confines nothing that three lower values do not also fail to confine. adocs/data/S148_rfp_ceiling_sweep.log for the grid, adocs/data/S148_sprt.log for the run. **Its four guards are asserted directly since 2026-09-09, S191**: `reverse futility does not fire at a node in check`, `... at a PV node`, `... above its depth bound` and `... inside the mate band`, one case each in `tests/test_search.cpp`, each establishing the guard's own condition at the node before the search and reading whether the rule fired. Only the negative edge of the mate band is constructible -- the positive one needs a static score near `MATE_MIN`, which `evaluate_expensive()`'s clamp to `LAZY_EVAL_MARGIN` makes unreachable, and it stays recorded as the inert half S145 measured. The in-check case needs `beta <= -32957` to bite at all, because in check the static score is the `TT_EVAL_NONE` sentinel and the guard is otherwise equivalent to its own removal. **Null move pruning guards both edges of the mate band since 2026-08-23, S165, and until then it guarded only the positive one.** **Both edges are asserted directly since 2026-09-09, S191**, by `tests/test_search.cpp`'s `a node at the positive edge of the mate band makes no null move` and `no defender node inside the mate band makes a null move` -- the second scoring all 104 rows of `adocs/data/S165_defender_set.tsv` with zero tolerance, which is the first time anything in `tests/` reads that file. The other three conditions of the block have their own cases in the same suite: `an in-check node makes no null move`, `a node with only kings and pawns makes no null move` and `a node whose parent already passed makes no null move`, and `a null-move fail-high against a mate returns the bound` holds the artefact rule that returns beta rather than the mate a pass produced. Each was observed red under the guard-removal mutant that matches it and green with the guard in place; the guards themselves are unchanged, and the engine's own node holds no probe code at all -- `negamax_at<PROBING>` is instantiated `<false>` everywhere the engine searches and `<true>` only at the node a test drives, because the branchy alternative measured **1.49 % of nodes per second, sd 0.66 % over 13 interleaved pairs**. Reverse futility has always tested `beta < MATE_MIN && beta > -MATE_MIN`; null move pruning tested the first half alone, so with `beta <= -MATE_MIN` -- a defender node inside a mate proof, where beta is a mate bound against the side to move -- any null-move result cleared beta and the node failed high on a reduced search's word. The reduced search is the instrument that misses mates and this rule hid a mate in two once already; no comment, decision or step ever recorded why the two rules differed. **Measured from the defender's side, which is the reading S145's sweeps cannot produce**: 104 defender nodes taken from `S145_mate_set.tsv`'s own `defender_nodes` column, each one's distance re-proved here by exhaustive AND/OR enumeration and corroborated by stockfish at 4000000 nodes -- 104 of 104 agreeing -- because that column walks the defender's *first legal reply* and not its longest, which makes the arithmetic `root_distance - (ply + 1) / 2` wrong on 3 of the 104. At depth `2k + 8` with both reverse futility bounds held, the guard takes the set from **80 of 104 exact to 82**, m2 28/31 to 29/31 and m3 2/15 to 3/15, with **0 false mates and 0 distances shorter than proved on both** and no delay regression anywhere. **And it is inert in ordinary play, counted rather than assumed**: over 400 corpus positions at depth 10 the negative band is reached on **0 of 301620** otherwise-eligible null-move nodes, against **3079 of 6252, 49.2 %** over the defender set. So `search_bench` is node-identical at depth 9 -- 121512 / 800769 / 62907, `c3d5` / `e2a6` / `d7c8q` -- and that is the reachability count restated, not a claim of behaviour neutrality: no bench position contains a mate bound. **H1 accepted** against `elo0=-5 elo1=0 alpha=0.05 beta=0.05` in **18598 games and 7 h 58 m 07 s** against `e918d05`, `LLR 2.96` past 2.94, `Elo 0.95 +/- 3.56`, `nElo 1.34 +/- 4.99`, `LOS 70.03 %`, `Ptnml(0-2) [688, 1883, 4161, 1824, 743]`, 50.14 %, DrawRatio 44.75 %, **0 time forfeits in 18601**. So a regression of **5 nElo** or more is excluded -- the bound is in normalized Elo, `fastchess.sh` passing `model=normalized`, and this run's own printed pair puts it at **3.54 logistic Elo** (`nElo 1.34` against `Elo 0.95`, ratio 1.41; the interval widths agree at 4.99/3.56 = 1.40) -- and no gain is claimed: the interval straddles zero and the reachability count is why it must -- an engine that never reaches the band in ordinary play cannot play differently there. **The run went nearly the full distance and that is the reading, not a defect**: a true effect at zero against a bound pair of [-5, 0] crawls to H1 rather than stopping early, which is DEC-063 from the other side. `adocs/data/S165_nmp_defender_sweep.py` is the sweep, `adocs/data/S165_defender_set.tsv` the proved set, `adocs/data/S165_sprt.sh` the run with its three readings pre-registered before the first game. `2026-08-22_adversarial-F04`. Its margin is **63 centipawns per remaining ply**, cut from the 75 S068 measured and the 100 S033 shipped unfitted: 14.5 % fewer nodes at depth 9 and, pooled over 11348 games, a small positive effect of about 3 to 5 Elo and not a regression of 5 or more (2026-08-17, S068; the bounds lesson is DEC-063). Aspiration windows search the root of each iteration from depth 2 in a band **21 centipawns** either side of the previous iteration's score, doubling the failing side alone and going to the full window past 437; S085 retuned all three from the 5 / 50 / 400 S021 shipped, and the values here are `AspirationMinDepth`, `AspirationDelta` and `AspirationMaxDelta` as they compile today. S021's schedule was chosen on node counts over 300 positions in three samples, where it costs 0.9248 of what the feature switched off costs and nothing else swept is below 0.9439, and one sample alone would have chosen a worse one. **H1 accepted over 824 games** against `elo0=-5 elo1=5`, `Elo: 35.12 +/- 19.06` -- the point estimate is biased upward by the early stop, so what the run establishes is the pre-registered claim, not a regression of **5 nElo** or more -- **3.99 logistic Elo** at this run's own pair, `nElo: 44.04 +/- 23.72` against `Elo: 35.12 +/- 19.06`, ratio 1.25 -- with the sign positive at `LOS: 99.99 %` (2026-08-17, S021) **Shallow-depth pruning entered the move loop on 2026-09-13, S109 (DEC-205)**: four rules over quiet moves at non-PV, non-check nodes past the first legal move and outside the mate band, all gated on the same reduction the move is searched with, `max(0, depth - lmr_adjusted_reduction(...))` through `lmr_depth_of` -- the table's ticks, the node-type terms and, since S236, the quiet's own history summed and rounded once, which is why a node type that reduces harder also prunes earlier (the gate read `lmr_reduction(depth, move_number)` when S109 landed; S098 verdict 2 gave it the node terms) -- late move pruning past a count of 7.33 moves, doubled when improving, flat in depth because the project's own census found the cutoff index falling with depth; futility at `static + 147 + 170 * lmr_depth <= alpha` on a static adjusted by a table score whose bound certifies a direction (S108's layer (c); the futility margin was its only consumer until S234); history pruning below `-576 * lmr_depth` on the raw butterfly entry, nearly inert until S222 lands the continuation table; and quiet SEE below `-50 * lmr_depth^2`; each covered to lmr depth 8 and each switched off exactly by its cap at 0, the bisection's legs. **The gives-check exemption binds all four, late move pruning included**: the published form without it lost `MATE_IN_2_B_POS`'s mate in two at depth 3 at every count below 30 moves, so DEC-180's red-guard clause fired and S218 folded in; the skip-quiets flag is set at generation and applied after `make_move`. Every rule has a direct guard test in `tests/test_search.cpp` and a mutant `tools/mutation_check.py` kills; the mate case `6qk/7p/2p2p1B/4R2P/4P1Q1/1p4P1/5P2/6K1 w - - 1 43` (Stockfish `#+2`, `e5e8`) was observed red with the guards removed. `bench` 27322394 -> 7111579; **decided by S109's one SPRT at `{0, 5}` nElo against `50e3661` on `noob_3moves.epd`: H1, LLR 2.97, `Elo +46.90 +/- 15.43`, `nElo +56.76 +/- 18.44` over 1364 games in 37 m 51 s, 0 forfeits (2026-09-13).** **Capture SEE pruning and an extra reduction ply entered the move loop on 2026-09-14, S091**: a capture whose exchange evaluation loses more than `50 * lmr_depth` is skipped under the same node guards as S109's four quiet rules, to lmr depth 8, switched off exactly by its cap at 0; and any move -- capture or quiet -- the exchange evaluation puts under zero is searched one ply shallower inside late move reduction's eligibility, a capture being unreduced by that rule so the ply is its whole reduction. The gives-check exemption binds both, reached on a capture through its own `is_check()` because `is_check_move` is hardcoded false there. Every rule has a direct guard test and a mutant `tools/mutation_check.py` kills; three mined-set mates whose lines run through a losing capture were observed red under named mutants. `bench` 7105111 -> 6267842; decided by S091's one SPRT at `{0, 5}` on 2026-09-14 (`adocs/data/S091_sprt.sh`, `adocs/data/S091_sprt.log`): **H1, `Elo 46.52 +/- 15.43` at `8+0.08` over 1360 games**, both rules kept; the stopping run's figure is upward-biased and the claim is at least 5 nElo (DEC-063). **One-ply continuation history returned with a scale of its own on 2026-09-14, S222 (DEC-194, DEC-198, DEC-209)**: the table of S024 verdict 1 -- `cont_hist[12][64][12][64]` on `search_state_t`, keyed on the previous move's (piece, to) and this move's, reached through one helper `continuation_entry`, written at every quiet cutoff and summed into `score_move`'s quiet return -- rebuilt on its own constants. `ContHistBonus` and `ContHistMalus` grade it in thousandths of its own band at chesso's median remaining depth 11, its bound is the `int16_t` ceiling as a definition rather than a setting, and `ContHistWeight` decides how much of the quiet band the term spans, so the band is `[-(QuietHistoryMax + ContHistWeight x 32767 / 100), +the same]` and still clears the countermove band by 100 at both declared maxima -- 688107 against 700000, asserted at both edges. (Its ceiling was halved to 1000 for S231's second weight on 2026-09-18, DEC-218, and returned to 2000 with that table's revert on 2026-09-20, DEC-224.) Three axes and not four because a bound and a weight over one table are one degree of freedom: DEC-194's two suspects, the doubled share and the shared bound, are the same statement about that span and the fit tests them on it. Ply 0 and the node after a null move pass no previous move and touch nothing, each asserted with the mutant that breaks it killed. History pruning still reads the raw plain entry. `bench` 6267842 -> 5950740, -5.1 %, at 3.6 % fewer nodes per second; the defaults above were first settings and **S222's own SPSA lane fitted them on 2026-09-14/15** -- `tools/spsa_s222.json`, `adocs/data/S222_spsa.sh`, eleven axes, 1250 iterations over 60000 games at 2+0.02 on `UHO_4060_v3.epd` so tuning and verification share no openings, 8 h 37 m, `adocs/data/S222_spsa_trajectory.tsv`. Every axis moved, so the run is not stuck and the SPRT is owed: `ContHistBonus` 15 -> 17, `ContHistMalus` 15 -> 18, `ContHistWeight` 25 -> 26, `QuietHistoryMax` 8192 -> 8831, `HistoryBonusQuad` 1 -> 6, `HistoryBonusLin` 0 -> 19, `HistoryBonusConst` 0 -> 2, `HistoryMalusQuad` 1 -> 0, `HistoryMalusLin` 0 -> 17, `HistoryMalusConst` 0 -> 36, `HistPruneCoeff` 576 -> 612 -- plain history's six coefficients and its band fitted for the first time in this project's history, the bonus quadratic at `6d^2 + 19d + 2` and the malus linear at `17d + 36`. **The weight landed near its seed**, the lane's own pre-registered reading that neither of DEC-194's suspects -- the doubled share, the shared bound -- is what the fit found, and the "at or under 5" row that would have owed a second attribution run is not triggered. `bench` 5950740 -> 5685915. The census on the fitted build reads 96.62 / 95.15 / 19.15 % against S024's 97.56 / 96.19 / 27.14, with a same-tree control at the incumbent vector at 18.94 % that attributes the fall to S109's and S091's pruning and not to the fit (`adocs/data/S222_census.txt`). **One gainer SPRT `{0, 5}` at the harness regime against the tree before S222, `d785b89`, decides the step as one vector under one verdict** (`adocs/data/S222_sprt.sh`, DEC-210): H1 keeps all eleven values and opens the two-ply table as its own step; H0 reverts the table and the vector to that baseline together, the eight never-fitted axes' fit becoming a step of its own. **Decided 2026-09-15: H1, `Elo 11.13 +/- 6.90` at `8+0.08` over 6278 games** (LLR 2.95, `adocs/data/S222_sprt.log`); the vector and the table stay, the claim is at least 5 nElo (DEC-063), and the two-ply table is S231. **Late move reduction scaled by the move's history measured zero and is not in the tree, 2026-09-16, S098 verdict 1 (DEC-213)**: `r -= clamp(hist_sum / LmrHistDiv, +/-LmrHistClamp)` through one helper that S109's gate shared was measured at three scales -- the band seed 8675 inert below depth 12 by node count, the two-axis lane's 699 / 3 at `Elo -2.92 +/- 5.02` over 11524 games, the census p90 1442 / 3 at `Elo -2.34 +/- 4.73` over 13078 games -- and never cleared `{0, 5}` at 8+0.08, both intervals mostly below zero; the term left in DEC-194's shape, the reduction and the gate read the table alone as before, `quiet_history_sum` stays as `score_move`'s reader, and the census, the lane and both runs are the evidence (`adocs/data/S098_v1_*`). **Late move reduction is adjusted by the node's type since S098 verdict 2, 2026-09-16 (DEC-214)**: `negamax_at` carries `cut_node` beside `is_pv`, so the pair names the three published node types by the prediction rules (the root PV; a PV node's first child PV and the rest CUT; a CUT node's first child ALL and the rest CUT; an ALL node's children CUT; the zero-window re-search keeps the scout's label, the full-window re-search under a PV parent is PV, the null-move child is the parent's opposite), with a Debug assertion that no node is both PV and CUT; the reduction becomes `r + LmrCutNode + LmrNotImproving + LmrTtCapture - LmrPv`, each 1 with range 0 to 2 (DEC-105 (c)) and 0 as an off value, clamped where the raw table was clamped, and **S109's shallow-depth gate reads the same adjusted number**, so the gate and the reduction are one value; at all four off values the helper returns the raw table and the engine is the one before the step, bench signature included. A firing census came first -- 21.12 / 52.75 / 24.83 / 26.72 % of 5478549 reduction sites at depth 12 (`adocs/data/S098_v2_node_census.txt`) -- so no term shipped inert; `bench` 5685915 -> 5469072. **Decided 2026-09-16: H1, `Elo 29.05 +/- 11.52` at `8+0.08` over 1990 games** (LLR 2.95, `adocs/data/S098_v2_sprt.log`) against `50fd965`, the tree verdict 1's removal left; the four terms stay at 1 ply each, the claim is at least 5 nElo (DEC-063), verdict 3 (the re-search rule) opens against `a771260`, and S127 refits the four beside the table's own constants and S109's thresholds. **The depth a reduced move's re-search runs at answers that search since S098 verdict 3, and it answers it only upward**: `lmr_research_depth` returns `child_depth + 1` where the reduced score cleared the node's own fail-soft best by `LmrDeeperMargin` with a reduction of at least `LmrDeeperMinReduction`, and `child_depth` otherwise; the base is the node's `best_so_far` before the move and never alpha, the returned depth is always strictly greater than the reduced depth, and the full-window re-search is untouched. `LmrDeeperMargin` at its range top is **not** an off value -- the fail-soft best sits below alpha at every scout node, so the condition still fires there -- and the rule's off value is `LmrDeeperMinReduction` at its own range top (DEC-215), where the re-search runs at `child_depth` everywhere and the engine is the one before the verdict, bench signature included. **A second path ran the re-search one ply shallower until 2026-09-18 and was removed after it was measured**: the pair read `Elo -9.97 +/- 7.56` over 4496 games, the whole interval below zero, and the pre-registered bisection's leg 1 switched the shallower path off and read H1, `Elo 5.75 +/- 4.37` and `nElo 7.44 +/- 5.65` over 14510 games against the same reference, so the loss was that path's. It left with `LmrShallowerMargin`, with the `reduction >= 2` guard written only for it and with the floor at 1 that only it could reach; the removal is behaviour-neutral at the shipped configuration and INV-6 proves it rather than asserting it -- `bench` 4646334 unchanged, `tools/search_bench.py` identical in every count and best move at depths 9 and 12, and the tune build at the rule's off value back at the reference's 5469072. **Decided 2026-09-18 (DEC-217): S098's three verdicts are spent** -- history scaling measured zero and is not in the tree, the node-type terms gained 29.05 and stay, the re-search rule lost 9.97 whole and gained 5.75 as its deeper half alone, which is what ships. **S095, 2026-09-20 to 2026-09-21 (DEC-222 clause 2)**: the node adjustment gains a fifth term, so the reduction is `r + LmrCutNode + LmrNotImproving + LmrTtCapture + LmrNoTtMove - LmrPv`. `LmrNoTtMove` is added where the node's table entry carries no move at all -- no entry, or one quiescence wrote without a move, which since S094 is the only way a moveless entry is made -- and it is 1 with range 0 to 2 (DEC-105 (c)), 0 being an off value at which the sum is the four-term sum and the engine is the one before the step, bench signature included (DEC-215). The step was written as a node-level depth cut and re-formed to this before it was implemented; that cut stays available as a later step. Decided by one `{0, 5}` nElo SPRT against the tree without the term (`adocs/data/S095_sprt.sh`): **H1, `Elo 5.92 +/- 4.50`, `nElo 7.51 +/- 5.70`, 14252 games, 0 forfeits** -- the term stays at its seed for S127 to fit; the claim is at least 5 nElo (DEC-063). `bench` 4646334 -> 4579468; two mate-carry short-line ceilings rose with it (DEC-225). **Singular extension, S097 verdict 1, 2026-09-21.** At a non-root node with no exclusion active, whose transposition entry carries a move, was searched to at least `depth - SeTtDepthMargin`, certifies a lower or exact bound and holds a score outside the mate band, the node is searched again at `(depth - 1) / 2` against the window `SeMarginPerDepth * depth` below that score, with the entry's move excluded, and never at a node whose ply has reached `SePlyFactor * depth`. If every alternative fails below that window the entry's move is searched one ply deeper -- one ply, once per node, and never recursively. A node searched under an exclusion takes no table cutoff, writes no entry, makes no null move, runs no reverse futility and starts no verification of its own; a node whose only legal move was excluded returns its own alpha rather than a mate or a draw (DEC-226). `SeMinDepth` 10, `SeTtDepthMargin` 4, `SePlyFactor` 5 and `SeMarginPerDepth` 9 are the settings, each DEC-105 (c); `SeExtend` 1 is the switch whose 0 is the tree before the step, bench signature included (DEC-215 clause 2); `SeMultiCut` is verdict 2's switch, described after verdict 1's result below. The entry's depth, bound and score are copied out beside the table move before any recursion, because the slot can be overwritten by a deeper node. `bench` 4579468 -> 5066204; at `go nodes 1000000` the three `search_bench` positions reach depths 16, 13 and 15 against 17, 13 and 17 before -- the cost the SPRT has to pay back, recorded (`adocs/data/S097_v1_sprt.sh`). Decided by one `{0, 5}` nElo SPRT against `5c76ea9`, 2026-09-21: **H0, `Elo -0.81 +/- 3.70`, `nElo -1.06 +/- 4.81`, 20080 games, 0 forfeits** -- a zero and not a loss, the nElo interval [-5.87, +3.75] centred within one nElo of zero. The extension alone does not gain 5 nElo; it stays at `SeExtend` 1, not as a gain, as the carrier of verdict 2, the multicut, and the whole block leaves the tree if that verdict reads H0 (DEC-227). **Verdict 2, the multicut, landed 2026-09-21.** A verification that fails high instead ends the node: where its score reaches this node's own beta, some move other than the entry's already clears a bar just under the entry's score at a reduced depth, and the node returns **that score** -- the fail-soft value the search found and never the window's own bound -- without searching a move; never at a PV node, never a value inside the mate band, and never against a beta inside the negative mate band, each pinned by a case and a mutant (DEC-141), the mate case being a mined row of "pruning does not hide a forced mate" observed red with the guard removed. `SeMultiCut` is 1 since verdict 2, the switch whose 0 is verdict 1's tree exactly, bench signature included (DEC-215). `bench` 5066204 -> 4493659; at `go nodes 1000000` the three `search_bench` positions reach 17, 13 and 15 against 16, 13 and 15 with the extension alone, one of the two plies the extension cost coming back. Decided by one `{0, 5}` nElo SPRT against the tree with the extension alone, `f02f59a`, 2026-09-22 (`adocs/data/S097_v2_sprt.sh`): **H1, `Elo 7.17 +/- 5.13`, `nElo 9.31 +/- 6.66`, 10470 games, 0 forfeits either side** -- the multicut gains at least 5 nElo over the extension alone (DEC-063); both rules stay at 1, the extension's zero on record and kept as the carrier of a measured gain (DEC-227); the six settings stay at their DEC-105 (c) seeds for S127 to fit. **A check extension was tried and left, S188, 2026-09-22 (DEC-228, DEC-230).** The in-loop form DEC-133 kept for one verdict after DEC-087 retired the pre-move-loop one: a move that gives check searched one ply deeper inside the move loop. Its first form -- every check, bounded by a ply guard and a one-ply budget shared with the singular extension -- read five plies lost at a fixed node budget and `bench` +94.6 % on the tree before any game, so DEC-228 re-formed it to the literature's safe-check gate (the move's own static exchange at threshold zero, on the moved piece's destination square) restricted to the last `CheckExtMaxDepth` plies before the horizon, which met the stated bar (`bench` +28.1 %, one ply lost) and was measured: one `{0, 5}` nElo SPRT against the tree without it (`adocs/data/S188_sprt.sh`): **H0, `Elo -13.84 +/- 8.95`, `nElo -17.71 +/- 11.44`, 3542 games, 0 forfeits** -- a loss, the interval wholly below zero. The rule, its three settings, its cases and its mutants left in the same step, `src/` returning to the tree before it byte for byte (`bench` 4493659, `search_bench` identical at depths 9 and 12); the two goldens the deeper tree had moved returned by the re-derivation scripts it gave them, which stay. The check-extension question has no successor step. The reported line's guarantee is again exactly the depth of the iteration that produced it, the singular extension not having moved it on the case that holds it. **The reduction is accumulated in fixed point, S236, 2026-09-23 (DEC-231); the history term was tried as a fraction of a ply and left, 2026-09-24.** The table holds ticks of a ply -- 1024 to the ply, a compile-time constant beside `build_lmr_table` -- the node-type terms are summed in the same unit, and the sum is rounded to whole plies once, at `lmr_adjusted_reduction`'s return, by `LmrRoundBias` added before the shift. The bias is 0, which floors as the old `uint8_t` cast did, and that is a measurement and not a choice (DEC-231): at 512 and at every bias from one tick up the engine lost the mate in "pruning does not hide a forced mate", so the accumulator is behaviour-neutral -- `bench` 4493659 with all eight replies identical to the tree before it, `search_bench` identical at depths 9 and 12 -- and stays as the precondition every fractional reduction term needs (S237, S238). S098 verdict 1's history term came back in that unit, `clamp(hist_sum * 1024 / LmrHistDiv, +/-LmrHistClamp)` subtracted on the same expression the move ordering scores a quiet with, seeded from a census of `quiet_history_sum` at depth 12 (DEC-105 (b), `adocs/data/S236_hist_census.txt`), and was measured twice against the tree without it at `{0, 5}` nElo (`adocs/data/S236_sprt.sh`, `S236_v2_sprt.sh`): at a two-ply clamp **H0, `Elo -1.60 +/- 4.25`, `nElo -2.05 +/- 5.44`, 15658 games** on a tree half again the parent's; at one ply **no verdict, `Elo 2.61 +/- 2.64`, `nElo 3.38 +/- 3.40`, 40000 games**, the stalled walk the pre-registration named, a zero by DEC-063. The term left with its two settings, its cases and mutants and the two mate rows mined on its trees, each predecessor restored; DEC-213 stands, and a pair for the idea would be S127's to fit against games, not a third run's to pick. The shallow-depth gate reads the same summed and rounded reduction through `lmr_depth_of`. **A two-ply continuation history table was tried and left, S231, 2026-09-18 to 2026-09-20 (DEC-224)**: `cont_hist2[12][64][12][64]` beside the one-ply table, keyed on the move two plies back, on three axes of its own, fitted with the one-ply three in one 60000-game lane (every axis moved, none at a bound, `ContHist2Weight` 26 -> 24), and measured as one vector against `3a649c0`, the tree before it: **H0, `Elo -2.65 +/- 4.82`, `nElo -3.40 +/- 6.20`, 12070 games**, 0 forfeits, the census having the table read non-zero on 27.51 % of quiet scores so the zero is the technique's and not inert wiring. `src/` returned to `3a649c0` whole -- table, axes, and the one-ply three back to S222's fitted 17 / 18 / 26 -- proved by `bench` 4646334 to the node; `ContHistWeight`'s declared ceiling is 2000 again (DEC-218's halving went with the second weight). The lane, census and SPRT evidence stay under `adocs/data/` (`S231_spsa_trajectory.tsv`, `S231_census.txt`, `S231_sprt.log`). The two removal verdicts DEC-222 (7) attached to an H1 are not owed. The one-ply table's own H1 (DEC-210) is untouched. |
| exchange evaluation | `see()` exact, `see_ge()` fast; quiescence declines losing captures |
| time management | the clock becomes an allocation for this move -- `remaining / movestogo` with a `movestogo`, and **5 % of what is left plus half the increment** without one -- and the allocation becomes two limits: a soft one at 60 % that decides whether to begin another iteration, and a hard one at 300 % that a timer is armed at and that stops the search inside an iteration. The hard limit is clamped to `remaining - MOVE_OVERHEAD_MS` and floored by the S036 rule, and nothing downstream raises it; the soft limit is clamped to it. The soft limit is then scaled after each completed iteration by three factors multiplied together: **4 % off per consecutive iteration with an unchanged best move, up to 8**; up to **+50 % for a score that has fallen 100 centipawns or more since the previous one**; and, **since 2026-09-21, S132**, from the iteration at `TmNodeMinDepth` on, `(TmNodeBasePct - share) * TmNodeScalePct / 100`, where the share is the percentage of the root's own nodes spent under the move about to be played, accumulated over every iteration and every aspiration re-search of this `go` -- a choice that ate most of the search is not in doubt and gets less clock, a spread over many moves gets more. **The 30 % floor is on the product.** The third factor's three settings are chesso's own (DEC-105 (b)): a census of 300 stratified positions from the S021 pick at `go depth 12` (`adocs/data/S132_node_share_census.py`) put the median share at 53.5 %, and the two constraints -- neutral at the median, the floor at a share of 100 % -- give a scale of 151 and a base of 120, the depth gate being `AspirationMinDepth`'s value; a scale of 0 makes the third factor 100 at every share, which is the rule's off value, proved on the tree (DEC-215). The counting behind the share is behaviour-neutral (INV-6: identical `search_bench` counts and best moves at depths 9 and 12, `bench` 5066204 unmoved, the root snapshot priced at -0.10 % +/- 0.48 over twelve interleaved pairs), and the multiplier was decided by one `{0, 5}` nElo SPRT against `778c7b0`, 2026-09-21 (`adocs/data/S132_sprt.sh`): **H1, `Elo 16.28 +/- 8.47`, `nElo 21.21 +/- 11.01`, 3822 games, 0 forfeits either side** -- the claim is at least 5 nElo (DEC-063); the three settings stay at their census seeds for S127 to fit, with the time-management family's second-control caveat (S085) written into that lane, and a second-control confirmation before they are called shipped is the owner's parked question. A time the GUI named with `go movetime` is not scaled. **`DEFAULT_MOVES_TO_GO` is gone**: a sudden-death control no longer divides the clock by a move count nobody sent. The twelve constants are in `src/search_params.hpp` (S073) and the base allocation is deliberately the number the old formula produced at `movestogo 20`, so the SPRT measures the split and the scaling rather than a re-tuned base. **SPRT H1 accepted** at `elo0=0 elo1=10 alpha=0.10 beta=0.10`, 500 games in 21 m 59 s against `1069cc6`, `Ptnml(0-2) [13, 44, 94, 63, 36]`, and **0 time forfeits in the 500** -- checked from the PGN filtered to this run, because the fastchess log is WARN-only and was empty. The point estimate `+45.42 +/- 23.47` is **not** the effect size: an SPRT stops early exactly when the observed effect has run favourable. Recorded verdict: not a regression, sign positive (2026-08-18, S089; DEC-071). **The hard timer's disarm is one decision since 2026-08-23, S163.** `stop_search_after_ms()` sleeps and then asks whether the search it was armed for is still current; that check and its store into the stop flag are under `session_mutex`, and `begin_search_session()` publishes the new session id and the cleared flag under the same lock. Two separate atomics were not enough: a timer preempted between its load and its store raised the flag after the next `go` had cleared it, and that search died at its first poll -- depth 1 and an instant reply for no reason, iteration 1 running on the local `never_stop` so a move was still produced. Stale timers are the normal case and not an edge, a move ending at its soft limit with its hard timer still sleeping when the next `go` arrives, every move of every game. **Measured red then green with one harness and one widening**: a scratch 500 us `sleep_for` between the check and the store -- widening the window that already existed, not inventing one -- gave **46 hits in 12328 iterations** before the fix and **0 in 23651** after, `tools/timer_race_stress.py` both times. The unwidened race did not fire: **513787 iterations over 15 minutes on the pre-fix build**, one pinned core and the machine otherwise idle, 0 hits -- and 530104 in the same 15 minutes on the fixed build, also 0. That is the recorded outcome and not a claim that it cannot fire: the window is nanoseconds and the fix rests on the interleaving, with the harness kept as the net. `command_test` cleared the flag without moving the session, so the last `go`'s hard timer was armed against its searches; it calls the same helper now. Behaviour-neutral at fixed depth and discharged on node counts: 121512 / 800769 / 62907 and `c3d5` / `e2a6` / `d7c8q` at depth 9, identical to `c59cf5e` (INV-6). `2026-08-22_adversarial-F05` |
...
| absent, search | razoring, extensions other than the singular one (S097 verdict 1, 2026-09-21; the safe check extension near the horizon was tried and left on 2026-09-22, S188), ProbCut, the node-level internal iterative reduction (S095 shipped the reduction-term form instead), delta pruning, quiescence per-move futility, capture history, correction history. **Forward futility, late move pruning, history pruning and quiet SEE pruning left this row on 2026-09-13, S109**, and the `improving` flag -- present as a definition since S108 with no in-search call site -- got its consumer there: late move pruning's count doubles when improving. Reverse futility left this row at S033 (2026-08-16) and aspiration windows at S021 (2026-08-17). **Measured 2026-08-19, this is what the row costs:** after `e4 e5 Nf3 Nc6 Bb5 a6` at `go movetime 250` chesso reaches **depth 12 on 1448572 nodes** where Stockfish reaches **depth 15 to 16 on 158837 nodes** — nine times the nodes, four plies shallower. DEC-081 is the decision that follows from it |
...
- ~~The "absent, machinery" row above has no plan steps behind it.~~
  **Discharged 2026-08-18 by DEC-071** and re-ordered 2026-08-19 by DEC-081 to
  ...
  S097 (verdict 1, the extension, landed 2026-09-21 and measured zero, kept as
  verdict 2's carrier under DEC-227; verdict 2, the multicut, landed the same
  day for its own SPRT), the
  quiescence
  ...
  S109 landed 2026-09-13 with the gives-check exemption on all four (DEC-205)
  and its SPRT decides it. **The `improving` flag has no step of its own**: it
  needs a static evaluation at every node, which the main search did not
  compute, so S092 was retired into S108, which supplies it; S109's late move
  count is its first consumer. DEC-081 is the reordering: the search block now
