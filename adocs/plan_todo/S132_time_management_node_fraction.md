...
src/chesso.cpp:426-479 — base = remaining/movestogo, or 5 % of remaining +
...
src/search_params.hpp:145-195.

**What per-root-move attribution needs.** The node counter is one global:
`search_state_t::explored_nodes` (src/data_structures.hpp:445), incremented at
negamax entry (src/search.cpp:420) and quiescence (:200), reset per depth
iteration (chesso.cpp:698), summed into `result.total_node_explored` (:776).
There is no root loop of its own — the root is `ply == 0` inside negamax's
shared move loop (src/search.cpp:635-777; root-only branches :769, :788). The
...
`search_state_t`, which is constructed per `go` (chesso.cpp:646) and survives
...
beside the S089 accessors (chesso.cpp:59-81, uci.hpp:144-153) for the probe.
...
- **S089 (done)** is the base: this multiplies its two scalers and changes
  neither, per excludes; the three meet at chesso.cpp:836-851.
