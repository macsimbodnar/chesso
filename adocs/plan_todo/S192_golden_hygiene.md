...
| 6 | `tests/test_mate_carry.cpp` `expected_mate_lines()` in "a mate score carried across searches keeps a line that reaches it" | 5, 7, 6, 1, 9 | floor on the `info` lines carrying a mate score per replayed game of `adocs/data/S170_cases.tsv` at the row's `go` budget; refuses vacuity | none -- `adocs/data/S170_replay.py` replays the games but reports short lines, not this count | `adocs/data/S192_mate_carry_floor.py`: the test's replay (Hash 16, one process per game, `position fen ... moves ...` per ply from `start`, the row's `go`), printing the count per game; margin **0**, the floor equals the count observed 2026-09-02 | any search change -- 21 of 22 search mutants moved it; every search step re-derives it and its stamp says so (DEC-142 Consequences) |
