...
| R6 | `tests/test_search.cpp` "the node budget is honoured exactly"; `tests/test_engine.cpp` "an infinite search answers only once stop arrives" | the first asserts `explored_nodes <= budget`; the second asserts one playable `bestmove` after `stop` and nothing about before | rename to "the node budget is never exceeded" and "a stopped infinite search answers with a legal move"; no living document quotes either title (grepped `DEV_MANUAL.md`, `MANUAL.md`, `adocs/specs.md`, `adocs/plan.md`, `adocs/status.md`, `tools/`) | none, a rename | none |
