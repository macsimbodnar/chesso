...
- - **The TODO.** The filter comment is src/search.cpp:462-466, the drop itself
  src/search.cpp:469 (`!in_check && !MOVE_CAPTURE` skips). Provenance verified
...
- - **Filter change, not generator change.** GEN_CAPTURES already emits quiet
  ...
  src/bitboard.cpp:207-223 (emit_promotions, Q/R/B/N), src/bitboard.cpp:229-244
  (`Type != GEN_QUIETS` emits them at src/bitboard.cpp:236), pinned path
  src/bitboard.cpp:261-263 -- by the INV-3 partition rule "Promotions count as
  captures whether or not anything is taken" (src/data_structures.hpp:130-133),
  the same fold every surveyed engine ships. The main search already searches
  them unfiltered through its captures stage (src/search.cpp:875; no capture
  test in the src/search.cpp:886-917 loop; LMR-exempt at
  src/search.cpp:960-961). The out-of-check qsearch filter is the one place
  they are invisible, so the change is one condition at src/search.cpp:469:
  drop only when `!MOVE_CAPTURE(m) && MOVE_PROMOTED(m) != TO_QUEEN` (TO_QUEEN,
  data_structures.hpp:148). A generator change instead would repartition INV-3
  and move underpromotions between main-search stages for nothing.
- - **Ordering.** src/search.cpp:486 scores survivors with capture_score()
  (src/evaluation.cpp:1125-1135); an EMPTY victim reads the deliberate 13th
  table entry, piece_values_abs[EMPTY] = 0 (evaluation.cpp:41-43; EMPTY = 12,
  data_structures.hpp:168), already exercised by quiet evasions
  (search.cpp:458-460). A quiet queen promotion therefore scores 0 - 100 =
  **-100**: after every non-king capture, before a king capture. Ship that. The
  score_move mirror (ORDER_CAPTURE + promotion bonus, evaluation.cpp:1151-1158)
  is not reachable from search.cpp -- piece_values_abs is file-local, teaching
  capture_score the bonus would double-count inside score_move
  (src/evaluation.cpp:1155), and a second constant in search.cpp is the
  CLAUDE.md band hazard. At most pawns-on-the-seventh moves are admitted per
  node, and Lynx #302 measured promotion-ordering variants at 0.4 +/- 7.7 --
...
- - **SEE applies unchanged, which is what `excludes` requires, and it prices a
  ...
  (src/search.cpp:481-484): victim gain 0 (src/bitboard.cpp:1203-1204), then
  `see_value[promoted] - see_value[pawn]` = +800 and the recapture target
  becomes the queen (src/bitboard.cpp:1209-1212); see() mirrors it
  (src/bitboard.cpp:1293, src/bitboard.cpp:1300-1307). promotion_t values
  coincide with the see_value indices (TO_QUEEN = 4 -> 900). A promotion onto a
...
- - **TT and draws.** A quiet promotion can now be a stored best_move; negamax
  ...
  src/search.cpp:872-873), matching where the generator emits it. Quiescence
  checks neither repetition nor the 50-move rule (only negamax does,
  src/search.cpp:621-655); promotions reset the halfmove clock like captures,
  so Weiss 8b8f9516's rationale carries over unchanged.
...
3. **Capturing promotions unregressed**: they pass src/search.cpp:469 today via
...
5. 5. **The change**: extend src/search.cpp:469; rewrite the
   src/search.cpp:462-466 comment to describe what is searched now (the accepts
   names this). The in-check path is untouched, so the two fast-suite
   quiescence mate cases ("a side in check may not stand pat",
   tests/test_search.cpp:993-1027, and "mate is recognised at depth zero",
   tests/test_search.cpp:1067-1079) gate as-is.
...
- - **Explosion is bounded by construction.** Each admitted move consumes a
  ...
  src/search.cpp:446 (MAX_QSEARCH_DEPTH = 19, src/search_params.hpp:106) holds
  regardless. The published explosion warning attaches to checks (talkchess
  t=60962); no engine record of a promotion-specific depth guard was found.
- - **Accepts wording, verified consistent.** Capturing underpromotions pass
  src/search.cpp:469 today via MOVE_CAPTURE and stay in; "underpromotions stay
  out" governs the non-capture arm only. The node-count sweep the accepts
  allows is Weiss #779's exact question, measured 0.02 +/- 1.50 there --
  queen-only is the right default and the sweep decides on nodes alone.
...
- **S022 (after)**: the delta/futility decision must price promotion gain
  ...
  src/search.cpp:481 gate's treatment of promotions is re-decided if ever.
...
- - **S130 (before)**: the TT-substituted stand-pat is orthogonal; a stored
  quiet-promotion best_move reads back through src/search.cpp:872-873
  unchanged.
