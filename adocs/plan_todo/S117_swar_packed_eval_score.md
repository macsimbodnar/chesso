...
not folded.** So the form this step packs is already the merged one-division
taper, and reproducing its single truncation exactly is what the packing
arithmetic is required to do.
...
(src/bitboard.cpp:596-604); INV-2's memcmp checks (tests/test_search.cpp:2798,
tests/test_engine.cpp:63) survive any layout byte-identically. Accumulation in
