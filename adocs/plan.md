...
1. S192  every golden in `tests/` named and scripted, the piece anchors re-derivable in-repo, the soft-limit scaling asserted on a constructed history (F03, DEC-142)
2. S205  `test_perft` parses four check columns it never compares, and 42 of 71 asset layers carry real values for two of them -- counted, or the dead fields deleted (found by S193's fast check)
