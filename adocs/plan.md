...
1. S209  `setoption` follows the protocol on names and check values, `Hash` refuses a value that is not an integer in full and says so in every build, and `clean-tt` joins the search before it clears the table; node-identical (F11, F12, F13)
2. S215  the one assignment S207's repetition rule hangs on is pinned by a test that reaches it through `search()`, and by a mutant that test kills -- found by the Tier-1 fast check over `23f926d` and verified by measurement: mutated to `history.size - 1` the whole fast suite stays green while `bench` moves 13 %
3. S216  the S159 killer-slot census stops silently measuring the wrong position -- its `promo-mess` row is the pre-S208 `KILLER_POS` and is now refused, and the reader scrapes only `info ... nodes` so the row above is reported under its name; found by the Tier-1 fast check over `8aff8ac`
