...
**With S109 the ledger holds twelve: mean 4 h 14 m, median 4 h 19 m, 117425 games in 50.90 hours, 2307.0 an hour across the set.** S109 is the first strength verdict on the new book and the shortest run in the ledger -- the block's effect is far above the pair, so the SPRT closed in 1364 games at 2162 an hour, in line with the two runs before it.
...
- S109  **the shallow-depth pruning block: H1, `Elo +46.90 +/- 15.43`, `nElo +56.76 +/- 18.44`, 1364 games in 38 minutes** -- late move pruning, futility, history pruning and quiet SEE pruning in the move loop at the reduction-adjusted depth, off in check, at PV nodes, on the first move and in the mate band, the gives-check exemption on all four after the published LMP form lost a mate in two (DEC-180's clause, S218 folded in, DEC-205); every constant chesso's own, the LMP count flat by the project's census; a mate case red with the guards removed, ten mutants killed, Debug self-play clean, `gate_extra` five stages after the sanitizer build was repaired; `bench` 27322394 -> 7111579. 2026-09-13.
- S226  **`TOOLCHAIN.md` describes the workstation it runs on** -- an eleven-subsection "The workstation, end to end" with every version, path and setting traced to a probe run here (gcc 13.3, LLVM 22 suffixed, ccache 4.9.1, hyperfine 1.18.0, samply 0.13.1 refused at `perf_event_paranoid` 2, `fastchess alpha 1.8.1 20260720-daa3ea2`, Stockfish dev-20260810, python-chess 1.11.2, Syzygy 3-4-5, governor `performance` read never set), the macOS content kept and labelled the MacBook's, the oracle section re-verified on this machine's binary and relabelled the workstation's. Documents only, done beside S109's SPRT. 2026-09-13.
- S218  **retired: folded into S109 by DEC-180's own clause** -- the published late-move form without the gives-check exemption lost `MATE_IN_2_B_POS`'s mate in two at depth 3 at every count below 30 moves, so the exemption became S109's fix under the TESTS rule and its question is answered by a red guard, not an SPRT (DEC-205). 2026-09-13.
...
1. S227  pixello and the debug GUI leave the branch -- the submodule, the `CHESSO_BUILD_GUI` option and target, `tests/debug_gui.cpp`, everything under `tests/assets/gui/` -- on the owner's instruction (DEC-207); no `src/`, first when the machine is free
2. S199  a fixed-rounds drift match against a pinned early-S105 reference after each block boundary, read as a trend -- first point after the S109 block, on the workstation (DEC-108, DEC-139)
3. S210  the seven low engine defects of the 2026-09-10 audit as one batch -- clock wrap, full history, `go infinite`, `movestogo 0`, the unstoppable first iteration, dead positions in quiescence, the bishops comment -- and the still-open `2026-09-04_adversarial-F01`; an SPRT only if F22's census finds reach (F17 to F23, DEC-171)
4. S223  the load boundary requires one king a side and refuses a position whose side not to move is in check, so no `position` line can leave a board without a king or let a supplied move capture one; seven test positions re-picked with the oracle's word, `position empty` gone, three no-king branches become assertions; node-identical, `No functional change` (2026-09-12_adversarial-F01; DEC-171 filler behind S109, DEC-197)
5. S213  the evaluation header's stale zero-weight blocks, the mate-band argument that names the wrong bound, two dead public entry points and two `<cctype>` calls on a signed `char`, with a test for the bound the argument relied on; `No functional change` (F26, F27, F33)
6. S225  `tools/gate_extra.sh` stops leaking `STAGES` into the fast label it runs, so its documented single-stage invocation passes its own test -- filler, no `src/` (found 2026-09-13 by S109's second-tier gate)
7. S228  a fast-suite test proves the `id name` build stamp follows the tree without a reconfigure (2026-09-12_plan_adversarial-F02, DEC-206) -- filler behind S225
