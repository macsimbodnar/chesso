...
The pending order owes **roughly 38 to 48 SPRT verdicts** once multi-verdict
steps are counted honestly (S022's two, the evaluation groups
per-term; five more since 2026-09-19 by DEC-222 -- the fixed-point block and the table-tightened estimate, both
spent, then S235, S237 and S238), the three the reduction rebuild took having been spent on
...
H0 read as a zero in 5 h 42 m -- plus one SPSA lane per completed block and S127's full run (DEC-222), one to three datagen nights,
...
**The ledger holds 31: mean 5 h 23 m, median 5 h 26 m, 354401 games in 167.38 hours, 2117.3 an hour across the set.** **Fast class**, an effect outside the bounds interval -- thirteen runs, mean **2 h 15 m**. **Slow class**, inside it, on a bound or a true zero -- eighteen runs, mean **7 h 39 m**.
...
The thirty-one split in two and the split is not luck — it is where the truth
...
- **Slow class, an effect inside the interval or a true zero**: S024 v1
  4 h 10 m, S097 v2 4 h 58 m, S098 v1 5 h 26 m, S108 5 h 27 m, S231 5 h 37 m, S235 5 h 42 m, S098 v1 leg 2
  ...
  longest row).
  **Mean 7 h 39 m.** Every one of the eighteen has an nElo interval that reaches
...
1. S235  a node pruned by reverse futility returns a point between its estimate and beta, weighted by one fitted parameter, instead of the estimate itself -- the first of four fail-middle sites, one per verdict (DEC-222)
2. S237  a child reads the reduction its parent applied to the move that reached it together with how the static evaluation moved across that move, and gives a ply back or up (DEC-222)
3. S238  each node counts its beta cutoffs and the parent reduces its later moves more when the child's count is high (DEC-222)
4. S112  quiescence skips a capture whose best case cannot reach alpha, per move, before the exchange evaluation is consulted
5. S131  quiescence searches non-capture queen promotions instead of filtering them out
6. S022  decide between delta pruning and the per-move futility S112 adds, by measurement -- deleting delta pruning is a valid recorded outcome
7. S113  a shallow verification search over good captures prunes a node whose score is already far above beta
8. S114  the null move reduction scales with how far the static score is above beta, and the base reduction is re-decided
9. S115  the widening schedule is re-swept fail-soft, a fail-low halves beta toward alpha, and a repeated fail-high costs the root a ply
10. S116  a node whose static score is hopelessly below alpha drops straight to quiescence, at depth one only
11. S202  a mate score inherited from the table at a depth too shallow to back it is given a line that reaches it or is not published as a mate -- the class S171's census measured, at a ceiling of 8 lines from 1 search in 3000 games and not a zero, free to alter play under its own SPRT where S171 was not (DEC-150)
12. S020  compute the in-check state once per node instead of once per call site
13. S055  taper mobility and king safety through one division instead of two, tightening the model guard's bound to 2
14. S117  the middlegame and endgame halves of every evaluation term travel in one integer instead of two
15. S120  a small cache of full evaluations by position key, so the score behind the lazy shortcut can be paid for once
16. S119  the table becomes cache-line clusters with an aged replacement, a prefetch issued when the key is known, and huge pages
17. S032  use _pext_u64 for sliding attacks where BMI2 exists, keeping magics as fallback
18. S030  move_t drops the moving piece and becomes 16 bits
19. S134  delete rook-on-the-seventh and passer bucket 5 by folding their weights into the piece-square tables, which is bit-exact, and shrink the parameter vector to 823
20. S082  the corpus labels a resolved position rather than the root -- the quiescence leaf, or the leaf reached by playing out a deep search's whole principal variation -- and samples few positions per game rather than many
21. S083  the corpus size and the generation node budget are decided by held-out error under a stated datagen budget, not by a volume target
22. S135  unfreeze the piece placement group and refit it, one bundled SPRT over the three remaining features, by the owner's decision of 2026-08-20
23. S136  unfreeze tempo, re-derive the truncation guard its zero weight holds one division down -- at two divisions once S055 has landed -- refit and resolve it at bounds that can
24. S121  mobility becomes a fitted curve per piece over a mobility area that excludes what a piece cannot safely stand on
...
26. S125  backward, phalanx, supported and weak unopposed pawns join the three terms that exist, each fitted
...
28. S101  evaluation terms for a piece attacked by a lesser piece, fitted like every other constant
29. S039  re-decide LAZY_EVAL_MARGIN from measured spread at the weights that ship today -- moved to sit directly before S122, its consumer, because four steps above change the sum it clamps (F16, DEC-172); `eval_spread`'s candidates include the shipping value (F25)
30. S122  king safety becomes a fitted linear accumulator with a quadratic finalizer, counting safe checks and weak squares, and it is no longer clamped
31. S124  the endgame half of the score is scaled toward a draw by what is actually on the board
32. S102  outpost and space terms in the evaluation, fitted like every other constant
...
35. S127  an SPSA run over the whole search parameter set as it stands after the search block, and an independent SPRT of what it returns
36. S129  three, four and five man tablebase probing, written from the format description
37. S152  **deferred, DEC-108; closes the main order** — the engine's absolute rating is re-measured once, near the 3000 mark rather than at a block boundary, at both time controls so S128's anchor-spread question is answered by the same run
38. S099  **reserve head, DEC-133** — a static evaluation correction learned from the difference between the static score and what the search returned, keyed on the pawn structure; the probe for the correction-history family, run on a spare night; S110 and S111 gated on its verdict
39. S023  **reserve, DEC-087** — history indexed by piece, target and victim, to order captures MVV-LVA rates equal
40. S025  **reserve, DEC-087** — retry searching losing captures after the quiets, now that capture history exists
41. S110  **reserve, DEC-087** — a second correction table keyed on the non-pawn structure, split by colour
42. S111  **reserve, DEC-087** — correction tables indexed by the move played two and four plies ago
43. S029  **parked, DEC-054** — a perspective network evaluation trained on chesso's own self-play
