...
1. S177  `rating.sh` and `build_release.sh` run on macOS as `fastchess.sh` does, and `rating.sh` arms its terminal marker before anything can fail -- today `./rating.sh --bracket` dies at `nproc: command not found` with no marker
2. S178  `movetext_to_san()` splits a move number glued to its move, so PGN import format (`1.e4 e5 2.Nf3`) builds a book instead of being refused at ply 0 -- found by running S174's fixed tools on that form; the shipped PGN has no such token, so nothing shipped is affected
...
12. S099  a static evaluation correction learned from the difference between the static score and what the search returned, keyed on the pawn structure
...
26. S042  set the en passant square only when an enemy pawn can take it, so transposing move orders share a hash
...
33. S039  re-decide LAZY_EVAL_MARGIN from measured spread at the weights that ship today
