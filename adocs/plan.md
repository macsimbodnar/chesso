...
1. S215  the one assignment S207's repetition rule hangs on is pinned by a test that reaches it through `search()`, and by a mutant that test kills -- found by the Tier-1 fast check over `23f926d` and verified by measurement: mutated to `history.size - 1` the whole fast suite stays green while `bench` moves 13 %
