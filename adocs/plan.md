...
1. S206  `truncation_scan`'s model-against-engine counts moved between S076 and 2026-09-10 at unchanged weights -- 135399/99/30 to 138331/105/33 over the same corpus -- and the commit that moved them is named or the earlier reading is shown wrong (found by S192)
2. S195  node-limited searches reproducible across `ucinewgame` in a fast test, and `bench` resets the table per position (F08)
3. S194  the UCI book path executed by the fast suite, the weighted draw seeded through `CHESSO_BOOK_SEED` (F06)
4. S151  a change that moves a pruning or reduction parameter has its verdict re-taken at a control at least four times longer before the number is banked, starting with S085's shipped vector
...
6. S185  every published figure the plan argues from carries its URL or the word unverified -- the Ethereal ledger commit and the Stash changelog behind the block order recorded, the mis-attributed zero-weight and tablebase figures corrected (F06)
7. S182  the cost line is priced from the ledger of runs since S105 by effect class -- 4 h 40 m mean per verdict, not 45 to 75 minutes -- and re-derived at every verdict-landing commit (F03, DEC-136)
8. S183  the Elo arithmetic is re-derived from tabled per-step inputs with the ledger's measured transfer ratio as a third discount, and "the midpoint clears 3000" survives only if the number does (F04, DEC-136)
9. S024  history indexed by the move played n plies ago and the current move
...
40. S039  re-decide LAZY_EVAL_MARGIN from measured spread at the weights that ship today
