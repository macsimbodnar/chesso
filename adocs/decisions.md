...
## DEC-221  2026-09-19  A technique taken from the analysis of literature and open-source resources is implemented from its description
Tags:         provenance, copying, workflow, dec-016, dec-104, dec-105, dec-134
Context:      The 2026-09-19 analysis of literature and open-source resources
              (`adocs/data/2026-09-19_search_technique_study.md`, reviewed in
              `adocs/audit/2026-09-19_study_review.md`) proposed a set of
              search techniques with their published effect sizes. COPYING
              already forbids taking source, tables or constants from any
              project (DEC-016, DEC-104, DEC-105, DEC-134); what the rules did
              not say is what an implementer of one of those techniques works
              from, and a technique that arrives as a description is only as
              original as the implementer's reading of it.
Decision:     By the owner, 2026-09-19. A technique taken from the analysis is
              implemented from its description -- the analysis's prose or a
              publication about the technique -- with its constants seeded in
              DEC-134's three forms, and the implementer's brief carries that
              description rather than pointing at any project's code. The
              completion stamp says so. The COPYING rule carries the sentence.
Rejected:     Adapting permissively licensed code where a licence consents
              (DEC-104 allows it) -- for the techniques of this analysis the
              owner prefers implementation from description, so that the
              originality claim is the implementer's own. Leaving it implicit
              -- DEC-014 says the plan is to read the documented state of the
              art and implement it here, and stating the mechanism once is
              cheaper than re-deriving it per step.
Consequences: Every brief for S095 (as re-formed), S234 to S238 and any later
              step drawn from the analysis carries the clause; the study and
              its review describe techniques and measurements, never code.
...
Context:      The 2026-09-19 study proposed eight plan changes and its
              ...
              open-source record later replaced internal iterative reduction
...
Decision:     By the owner, 2026-09-19. (1) **The correction-history family
              ...
              **S095 keeps its id and changes form**: one more ply of
              ...
              Elo-per-game rows in the whole ledger the analysis read. (5) **Four
              ...
              form the open-source record carries today, each briefed under
              DEC-221's from-the-description clause; N7 waits for a threat map shared with S101,
...
## DEC-224  2026-09-20  The two-ply continuation history leaves the plan on a measured zero; the one-ply table and its fitted scale stay
Tags:         search, move-ordering, history, sprt, measurement, dec-210, dec-194, dec-222
Context:      S231 built a two-ply continuation history table beside S222's
              one-ply table (DEC-210 opened it on S222's H1), fitted all six
              continuation axes in one 60000-game lane on `UHO_4060_v3.epd`
              (every axis moved, none touched a bound, `ContHist2Weight`
              26 -> 24 in the pre-registered "near 26" reading), and measured
              the fitted vector against `3a649c0`, the tree before the table,
              as one gainer SPRT `{0, 5}` nElo at 8+0.08 on `noob_3moves.epd`
              -- one vector under one verdict, DEC-210's reading. H0 at
              12070 games: `Elo -2.65 +/- 4.82`, `nElo -3.40 +/- 6.20`,
              LLR -2.95, 0 forfeits. The nElo interval [-9.60, +2.80] has
              its centre below zero and its top short of the bound, the
              shape DEC-194 did not keep. The census on the fitted build
              read the two-ply table non-zero on 27.51 % of quiet scores
              against a same-tree control, so the zero is about the
              technique as built here and not inert wiring. Its cost in
              the tree was real: 1.125 MiB more on `search_state_t` (2.33
              MiB total, half the test stack's headroom gone), -3.84 % of
              nodes per second, one more parameter through every
              `negamax_at` recursion.
Decision:     By the coordinator, 2026-09-20, applying the pre-registration
              the owner's rules bind (`adocs/data/S231_sprt.sh`, written
              before a game was played); the owner may overrule. `src/`
              returns to `3a649c0` whole: the table, its three axes, and
              the one-ply three back to S222's fitted 17 / 18 / 26 -- which
              are exactly `3a649c0`'s values, so no axis is left without a
              verdict and S222's own H1 (DEC-210) stands untouched;
              `ContHistWeight`'s declared ceiling is 2000 again, DEC-218's
              halving having gone with the second weight it was made for. The
              two-ply idea leaves the plan; the lane, census and SPRT
              evidence stay under `adocs/data/`. The two removal verdicts
              DEC-222 (7) attached to S231's H1 -- killer slots, then the
              countermove table -- are not owed, because the stack they were
              to be measured against did not ship; the selection rule of
              DEC-222 (7) still applies to any later step that makes them
              redundant. The S192 node-budget golden, outside the middle
              half of its band since S222, is re-derived on the reverted
              tree in the revert commit (DEC-142).
Rejected:     Keeping the table as a measured zero (S005, S006, S015) -- it
              costs memory, nodes per second and complexity for no measured
              gain, and the pre-registration chose the revert before the
              number was known. Keeping the fitted one-ply values 18 / 17 /
              24 without the table -- fitted with the table present, they
              have no verdict of their own; `3a649c0`'s 17 / 18 / 26 do
              (DEC-210). A second lane or a longer control before deciding
              -- the verdict is the pre-registered one, and a technique
              re-measured until it passes is the practice DEC-019 exists to
              refuse. Re-trying at a different key (four plies, in-check
              axes) -- a new idea with a new file, if the plan ever wants
              it, not a continuation of this one.
Consequences: S231 completes on the zero: stamp, `plan_done/`, the Open list
              loses its head, `plan.md`'s ledger regenerated by
              `tools/ledger.py` with the first DEC-220 row. `specs.md`'s
