...
done:       2026-09-22 02:05 -- two verdicts off one verification search, both landed and both measured. **Verdict 1, the extension** (`88ec74f`, 2026-09-21): `negamax_at` gains a trailing `excluded_move`; at a non-root node whose table entry carries a move, was searched to at least `depth - SeTtDepthMargin` and certifies a lower or exact bound outside the mate band, a verification search at `(depth - 1) / 2` against a window `SeMarginPerDepth * depth` below the entry's score, with the entry's move excluded and never at a ply past `SePlyFactor * depth`, extends that move by one ply when every alternative fails below the window; a node under an exclusion takes no table cutoff, writes no entry, makes no null move, runs no reverse futility and starts no verification of its own, and a node whose only legal move was excluded returns its own alpha (DEC-226); the entry's three fields are copied out beside the table move before any recursion, the first fast check's one real finding. Four settings at DEC-105 (c) with `SeExtend` the off switch proved to the node against the parent (DEC-215 clause 2); fifteen cases and eighteen mutants (DEC-141), twelve older anchors re-pointed and re-run; node counts move by construction, so INV-6 does not apply and the SPRT decides. `{0, 5}` nElo against `5c76ea9`, pre-registered in `adocs/data/S097_v1_sprt.sh`: **H0, `Elo -0.81 +/- 3.70`, `nElo -1.06 +/- 4.81`, LLR -2.96, 20080 games in 9 h 27 m, 0 forfeits** (`00e0209`) -- a zero and not a loss, read under DEC-227 as the pre-registration's own exception: the block stayed unflipped as the carrier of verdict 2, because the one-line flip was a guarding pass over every case and mutant that verdict 2 would undo either way, and because the extension's gain paying for the verification search made the multicut a pure-margin question. **Verdict 2, the multicut** (`63ad848`, 2026-09-21): one default, `SeMultiCut` 0 -> 1, the rule having shipped inert; where the verification fails high at or above the node's beta the node returns the fail-soft verification score without searching a move -- never `singular_beta`, never a value inside the mate band, never against a beta inside the negative mate band, never at a PV node -- each pinned by a case leg and a mutant (E20 to E23; the third leg and E23 added after the second cold fast check found the `beta > -MATE_MIN` term unpinned); the off-value case moved to the tune build; the accepts' mate row mined by `adocs/data/S097_mine_mate_row.py` from 269 of this project's own labelled mates (`4N3/8/3P1ppk/4p2p/4P2P/1n1P2P1/Q4PK1/3q4 w - - 5 46`, mate in 5 at depth 14, Stockfish `#+5`) and observed red with the guard removed; mutants E01 to E23 killed 22 of 22 in one clean run after E05, orphaned by verdict 1's copy-out, was repaired; goldens re-derived by their scripts and none moved (`golden_defaults` 60 rows, `capture_mates`' depths, the mate-carry ceilings); `bench` 5066204 -> 4493659 with every `bestmove` unmoved; fixed-node depths 16/13/15 -> 17/13/15. `{0, 5}` nElo against `f02f59a`, pre-registered in `adocs/data/S097_v2_sprt.sh`: **H1, `Elo 7.17 +/- 5.13`, `nElo 9.31 +/- 6.66`, LLR 2.95, 10470 games in 4 h 58 m, 0 forfeits either side** (`4dc0b8c`) -- the claim is at least 5 nElo (DEC-063); both rules stay at 1, the extension's zero on record and kept because it carries a measured gain (S005, S006, S015; DEC-227). Debug self-play 8 games with 0 `Assertion` and `tools/gate_extra.sh` 5 stages green on each landing (DEC-141); the fast suite 40 of 40 in both builds and the format check clean at every commit; the two cold fast checks each earned their keep. The six settings stay at their seeds for S127 to fit. `bench` 4579468 -> 5066204 -> 4493659. Implemented from the step file's own description and the published record (DEC-221); no other project's code was opened; the record's +11 to +20 was direction only (DEC-019).
