...
blocks:
paused_by:
done:      +21.10 +/- 10.47 Elo over 3396 games, H1 accepted at LLR 2.95 against a2f0065; gate green 12 of 12 fast plus test_perft, format clean
...
```
REF=a2f0065 CONCURRENCY=12 ./fastchess.sh
candidate 33aa3b4  reference a2f0065  tc 10+0.2  concurrency 12 of 12

SPRT ([0.00, 5.00]) completed - H1 was accepted
Elo: 21.10 +/- 10.47, nElo: 23.61 +/- 11.69
LOS: 100.00 %, DrawRatio: 35.51 %, PairsRatio: 1.27
Games: 3396, Wins: 1466, Losses: 1260, Draws: 670, Points: 1801.0 (53.03 %)
Ptnml(0-2): [235, 247, 603, 303, 310], WL/DD Ratio: 9.05
LLR: 2.95 (100.3%) (-2.94, 2.94) [0.00, 5.00]
Total Time: 02:30:12
```

**+21.10 +/- 10.47 Elo, H1 accepted. The paste is kept.**

**Run integrity.** Zero lines over the whole log matching illegal, disconnect,
timed out, terminated, assertion or segmentation. Every game ended in
adjudication (2888), three-fold (257), fifty moves (134), insufficient material
(115) or stalemate (3).

**Attribution.** `git diff a2f0065 33aa3b4 -- src/` is fitted values only — the
five piece defines, both piece-square tables, and the mobility, king safety,
passed pawn and pawn structure arrays. No other line of engine source differs
between the two players. `tools/tuner.cpp` and `tools/tuner_groups.hpp` also
differ, by the `--freeze` flag, and neither is linked into the engine binary
that plays. DEC-020 is why this is checked rather than assumed.

**What the number is and is not.** It is what the 827 constants are worth
together against the constants that shipped before them, at 10+0.2 on this
machine, from one corpus generated by one engine. It is not a valuation of any
single constant, and it does not rank the two fits against each other — the
unfrozen fit was never played. The held-out error, 0.122560 to 0.118460, ranked
nothing and still ranks nothing; this is the figure that decided it.
