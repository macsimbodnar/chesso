...
done:       2026-09-13 03:30. **Design (iii) ran as pre-registered and the vector's gain is not gone at four times the control; how much of it transfers, this estimate cannot say.** `adocs/data/S151_ltc.sh` launched 2026-09-12 23:37 on the idle workstation under governor `performance` (DEC-195): candidate `21b4a21` against its parent `3488506`, both built fresh by `build_ref`, `32+0.32`, `Hash=64`, `noob_3moves.epd`, seed `20260912233659`, 1000 pairs, no bounds. `SPRT-RUN-DONE` at 03:17: 2000 games in 3 h 40 m, **545 an hour** against the pre-registered 528 (110.6 plies and 78.2 s a game, S105's 98.0 plies at `8+0.08` grown as section 4 expected). fastchess's final block: **`Elo: 11.99 +/- 11.57, nElo: 15.81 +/- 15.23`**, LOS 97.90 %, DrawRatio 36.10 %, PairsRatio 1.22, `Ptnml(0-2): [83, 205, 361, 262, 89]`, W 625 L 556 D 819. Read as pre-registered and never as a verdict: against zero the logistic interval [+0.42, +23.56] excludes it at its edge; against the `8+0.08` verification's `Elo 21.02 +/- 9.86` the difference is 9.0 on a combined error of 15.2, so the two readings agree within noise and the estimate separates "the gain transfers" from "it is gone" -- it is not gone -- and cannot separate the whole +21 from half of it, which is what section 7 said 1000 pairs would buy. The expected half-width held: +/- 11.57 observed against the +/- 11.6 derived from the `8+0.08` pair variance; the pair variance at `32+0.32` is 0.2876 (sd 0.5362; `adocs/data/S151_ltc_pairs.txt`), draws 41.0 % of games, 1349 adjudications and 651 natural ends. Abort rule never approached: 0 time forfeits on either side (`tools/forfeit_report.py --max-pct 1.0`). `Incomplete mating PV` warnings: 57 from the candidate, 67 from the reference, both commits predating S147, S170 and S171, recorded and not a stop. **Nothing in the engine changes** (excludes); no axis is attributed. The rule stands as the harness half wrote it (DEC-202, `fastchess.sh`, `DEV_MANUAL.md` "The longer control, once per block", `specs.md` INV-6): the next reading is at the S109 block boundary beside S199's drift point. No guard test, mutant, Debug self-play or INV-6 node-count run was owed (section 6); the gate over both builds is in the completing commit. Evidence: `adocs/data/S151_ltc.log`, `adocs/data/S151_ltc_pairs.txt`; the full log and PGN stay gitignored under `.tuning/`. Machine-local throughput at `32+0.32` recorded in `.moltke.local.md`. Read and stamped by the coordinator; the harness half by an Opus 5 subagent and a repair agent (DEC-199), see "Harness landed".
