...
done:       2026-09-25 01:55 -- one verdict, H1. **Reverse futility is decided on S109's table-tightened estimate and returns that estimate less the margin**, behind `RfpTtEstimate` 1 (`169b4cb`, `bench` 4493659 -> 4803214, +6.89 %, one of eight `bestmove` replies moving): the entry's score where its bound type certifies the direction of its gap from the static evaluation, never inside the mate band, never at a node in check, never from an exact entry -- S109's rule with a second consumer and no new condition, its futility site reading the same number at either setting. The raw static evaluation stays what the entry's evaluation field and the improving flag's per-ply slots hold, asserted in every case, and no history update in this engine reads a static evaluation at all. **The step's `accepts` named two sites that do not exist on this tree**: the null-move block has no static-score condition (its guards are the node and the window, and scaling its reduction by the static score is S114) and razoring does not exist until S116, whose site joins under its own verdict; so the routing is one site, which is what the switch is named for. Decided at `{0, 5}` nElo against `5180a10`, the tree without the routing: **H1, `Elo 4.73 +/- 3.74`, `nElo 6.09 +/- 4.81`, LLR 2.98, 20062 games in 9 h 32 m, 0 forfeits either side** (`13b45cf`; the ledger's thirtieth row, `5edc45a`), the slow class by DEC-223 because the interval [1.28, 10.90] holds the bar; the stopping Elo is upward-biased and not the effect size (DEC-063). The switch ships at 1 and stays in the parameter set for S116 to re-ask the question with a second site; its 0 was proved the parent's tree to the node twice, the Release default and the tune binary at `setoption` (DEC-215). **The price, said here rather than left to the ledger**: in the cold fixed-depth regime the candidate finds 489 mate cells against the parent's 497 over S095's 141-position set at depths 3 to 12 (`adocs/data/S234_remine.log`), and S095's guarded row in "pruning does not hide a forced mate" was re-mined by its own script on this tree -- same position, same distance, depth 11 -> 8, the old depth kept in its GOLDEN block (DEC-142) -- a reading of a different tree and not of a worse engine (DEC-019), priced by this run; through iterative deepening the candidate reports that row's mate at every depth from 3 to 12 with node counts identical to the parent's. S103's case had its filler plant repaired without weakening (the leg's precondition moved, its assertion did not). Six cases on `rfp_estimate_drive_t` with preconditions asserted inside the drive, mutants G01 to G04 4 of 4 killed on the repaired tool (S239) and the tune-only red observed by hand; a cold fast check before landing found no defect; Debug self-play 8 games with 0 `Assertion`, `tools/gate_extra.sh` 5 stages green (1038 s) on the landing tree after a first run flagged a `plan.md` sentence of the coordinator's (DEC-141); the fast suite 40 of 40 in both builds and the format check clean at every commit. `MANUAL.md` (the option row, traced to the reverse-futility block) and `DEV_MANUAL.md` (the node-signature ledger) landed with `169b4cb` and need nothing at completion. Implemented from the study's row N1 and this file's description (DEC-221); no other project's code was opened and no constant of one is behind any value, because the rule has none to seed (DEC-134); the record's +6.07 was direction only (DEC-019), and S130's zero on the narrow quiescence form stands beside this H1 on the wide one (DEC-103).
