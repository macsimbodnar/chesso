...
blocks:
paused_by:
author:     Claude Opus 5 (agentic session, 2026-09-10)
done:       2026-09-10. **Three cases hold what an SPRT cannot see, and two of the guide's four predictions about them were wrong -- both found by running the mutants rather than by reading the code.** `tests/test_engine.cpp` gains, in `TEST_SUITE("engine: uci go")`: *node-limited searches repeat across ucinewgame* -- two sequences at ten node limits, each run twice with `ucinewgame` between, **40 searches and every pair identical** on the last `info` line's `nodes` and on the `bestmove` line, with ten and nine distinct counts against a floor of five so the sweep is not one search repeated; *a repeated go depth is cold only across ucinewgame* -- **77612 cold, 77612 cold again, 8460 warm** at depth 8, the inequality holding at every depth 5 to 10 (15905/683, 21794/7400, 51667/11926, 77612/8460, 121530/9084, 208806/8218) so no later depth change makes it vacuous; and *bench searches its last position cold* -- `bench 9`'s eighth position reads **128048** and a standalone `position mate2b` + `go depth 9` reads 128048. Green on the first run, as the guide predicted. **Four mutants, every one built and observed, all reverted before the commit.** M1, `tt_reset` out of `reset_for_new_game`: case 1 **red at the first limit**, `113 then 232`, cases 2 and 3 green. M2, `set_position`'s `initial_position != fen` body never runs: **all three green**, where the guide predicted case 2 red on `cold == cold_again` -- the prediction does not follow at HEAD, because S189's `reset_for_new_game` calls `tt_reset` **directly** as well as through `set_position(DEFAULT_POSITION)`, so `ucinewgame` clears the table whatever `set_position` does. M3, the gate deleted so every `position` resets -- the F08 hazard silently repaired, which is what `excludes:` forbids: **case 2 red**, `warm repeat 77612 equals cold 77612`, and that is the mutant the inequality half exists for. M4, both reset paths cut: **all three red** -- case 1 at 113/232, case 2 at 8460 against a cold 77612, case 3 at 479 against 128048 -- so case 2's equality half and case 3 have teeth, they just need both mechanisms gone. **The bench clause needed no code and case 3 cannot prove what it was written to prove.** S189 landed first, so `command_bench` already calls `reset_for_new_game()` inside its position loop and the accepts' "this step adds it otherwise" is void; **no `src/` change, no `Bench:` line, no SPRT.** Commenting that call out leaves `bench 9`'s whole output **byte-identical** -- every `nodes`, `score` and `pv` field on all eight positions and the signature total `1600876`, only `time` and `nps` moving -- because the eight FENs are pairwise distinct and `set_position` already resets on a differing FEN. The loop's reset guards a bench list that ever *repeats* a position, and no test can construct that list from outside; case 3 ships as the end-to-end coldness property with exactly that named at its site, plus the message that a red after the bench set is reordered means the case is comparing the wrong position. `bench 9` and not the bare form: same reset in the same loop for **0.21 s against 3.6 s**, and the bare form already has `tests/test_uci_surface.cpp` "bench prints one final signature line and repeats its total". **Section 4's constants were re-derived here and the MacBook's did not transfer** -- 77612 against its 77617, 121530 against its 121512 -- which is S203's key redraw landing in between and is why nothing in these cases is compared against a recorded number. **Goldens: none (DEC-142).** Every assertion compares two runs inside one process and no count from any run is written into the file; the `per_position.size() == 8` precondition mirrors the existing surface case's `first_best_moves.size() == 8` and is a structural count, not a golden. Cost: Release **0.343 / 0.021 / 0.205 s**, Debug **7.01 / 0.47 / 5.19 s** inside a 65.26 s `test_engine` against that build's 600 s ceiling. Gate **33/33 in both builds**, format clean under `CLANG_FORMAT_MAJOR=22` (DEC-146), `tools/plan_prose_check.py --touches` 0 flagged over 58 files. T1 of the S193 guide confirmed and noted at the site: `ucinewgame` sets the stop flag and only the `go` that follows clears it, which is why every search here goes through the UCI layer. DEC-141 owes nothing further -- no `make_move`, generator or search change, so no Debug self-play and no `tools/mutation_check.py` mutant. **Section 10, answered:** Q1 void, S189 landed; Q2's reading confirmed and implemented -- the last `info` line's `nodes` and the `bestmove` line, never `pv`, `time` or `nps`, and the count sits under the budget when the final iteration aborts without a PV (2917 at `go nodes 5000` here too); Q3 confirmed by measurement, a negative control for case 1 is not reliable -- without `ucinewgame` the warm repeat matched the cold count at 50000 on both sequences and at 2500, 5000 and 10000 on the second; Q4 the owner's decision of 2026-09-10, **neither MANUAL.md sentence is wanted**, both filed as findings, so the SURFACE golden is untouched and `MANUAL.md` is checked-and-unchanged. Docs: `DEV_MANUAL.md` "Measure" gains the warm-table paragraph beside the `tools/search_bench.py` block, naming both new cases; `README.md` untouched, human-owned. `adocs/specs.md` is not in `touches:` -- proposed for the coordinator, one sentence beside the `tests/test_mate_carry.cpp` paragraph: *a node-limited search repeated across `ucinewgame` visits the same tree, and a `position` repeating the FEN already loaded does not reset the table, both held by `tests/test_engine.cpp`*. Closes `2026-09-04_test_review-F08` with S203, which took the key half (DEC-154); the LMR `std::log` exposure stays recorded and unrepaired.
