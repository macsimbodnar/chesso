...
done:       2026-09-14 10:30. **Ten defects closed in two commits, and the one that alters the tree is not a regression.** The first half (`9075bf8`): F17 the halfmove clock saturates at 255 (`history_entry_t` stays 16 bytes, DEC-208), F18 a `moves` list past `POSITION_MAX_PLIES` 4871 is refused and the whole `position` command is transactional (`load_position()` then one `commit_position_base()`, so a refused line leaves the table, the remembered base and the book flag untouched), F19 `go infinite` clears every finite limit and the search holds open on `stop` (2026-09-04 F01 with it), F20 `movestogo 0` is sudden death, F21 the first iteration honours `stop` and the hard timer with a legal move still answered, F23 the insufficient-material comment names FIDE 5.2.2 and 6.9 and the accepted omission, 2026-09-04 F02 a bad `moves` token refuses the whole command, F03 a carried `root_move_hint` replaces the table premise at the root, and every `go` number takes S209's whole-token rule (DEC-208) -- each red first, `No functional change` on the bench and all six `search_bench` rows, Debug self-play 88 games clean. The second half (`9ef06f3`): F22, quiescence returns a draw for the position a move leaves when `is_insufficient_material()` holds, unconditional, red first on KvK, KNvK and KBvK with 24 preconditions; the reach census over 11503 positions of the S219 A/A at depth 10 found 0.062 % of quiescence moves landing on a dead board and **194 of 11503 root answers moved (1.687 %)**, so DEC-107's census discharge did not apply and the pre-registered `--nonreg` SPRT (`adocs/data/S210_f22_sprt.sh`, candidate pinned to `9ef06f3`, reference `9075bf8`, `{-5, 0}` nElo at `8+0.08` on `noob_3moves.epd`, seed `20260913205758`) ran 2026-09-13 20:58 to 2026-09-14 10:14: **H1, LLR 2.98, `Elo 0.01 +/- 3.17`, `nElo 0.02 +/- 4.03`, 28598 games in 13 h 16 m 30 s at 2155 an hour**, `Ptnml(0-2) [1422, 3156, 5186, 3069, 1466]`, 0 forfeits either side, crash census quiet, pair variance 0.3108 -- not a regression of 5 nElo, the point estimate at zero, as a rule that scores dead positions correctly should read. **The change-specific reading**: 1661 games ended by insufficient material and **0** of them carry a mate score in any move comment (the reader mapped the log's game numbers to the PGN's rounds as pairs; its first pass mapped them as games and named one wrong hit, corrected before this stamp). `Bench` 7111579 -> 7105111 at F22. Second tier: `tools/gate_extra.sh` five stages on `da0cfed` (1042 s); mutants not owed (no new pruning rule). Three fast checks over the two halves found no code defect; their real items -- a vacuous one-node test under a stale `stop` flag, atomicity that was `game`'s only, a duplicated 4871 golden, the dead score's frame, the census script's unchecked bench agreement, an uninstrumented abort clause -- were repaired before each commit and are recorded in the sections above. `MANUAL.md` first, the surface golden after; `specs.md` carries six passages; DEC-208 the three choices. Evidence `adocs/data/S210_f22_sprt.log`, `S210_f22_sprt_pairs.txt`, `S210_f22_census.txt`, `S210_f22_changed.tsv`, `S210_depth1_latency.py`. Implemented by two Opus 5 subagents and two repair agents; run, read and stamped by the coordinator (DEC-185, DEC-199).
