...
done:       2026-09-21 19:40 -- two increments in one landing (`474c288`): per-root-move node buckets in `search_state_t`, filled at the root loop only (a snapshot of `explored_nodes` before `make_move`, the delta added after `unmake_move` and before the abort check), behaviour-neutral by INV-6 -- identical `tools/search_bench.py` counts and best moves at depths 9 and 12 against the tree before, `bench` 5066204 identical, the snapshot priced at -0.10 % +/- 0.48 over twelve interleaved bench pairs (DEC-083); and the soft limit scaled from `TmNodeMinDepth` on by `(TmNodeBasePct - share) * TmNodeScalePct / 100`, the 30 % floor moved onto the product of the three scalers, the hard clamp and `go movetime` untouched. Seeds are chesso's own at DEC-105 (b): a census of 300 stratified positions from the S021 pick at `go depth 12` (`adocs/data/S132_node_share_census.py`, median share 53.5 %, quartiles 35.8 and 73.0) with the two constraints -- neutral at the median, the floor at a share of 100 % -- giving `TmNodeScalePct` 151 and `TmNodeBasePct` 120, `TmNodeMinDepth` equal to `AspirationMinDepth`; a scale of 0 is the off value, proved by the probe on the tree with the factor 100 at every share (DEC-215; the bench cannot see a clock rule, so its equality is necessary and uninformative). Cases in `tests/test_search.cpp` (the buckets account for every node; the accumulation across `search()` calls) and `tests/test_engine.cpp` (the factor monotone with its swing exactly the scale; the loop's floor on the product with the precondition that the raw product is under it; the depth gate; the clamp; the whole-search identity; `go movetime` unscaled), every precondition asserted inside the drive; mutants K01 to K07 killed 7 of 7 by `tools/mutation_check.py`, all seven invisible to the bench signature as the file claims. Fast check by a cold reviewer before the landing: no real defect, four stale counts fixed. Debug self-play 8 games with 0 `Assertion` and `tools/gate_extra.sh` 5 stages green on the landing (DEC-141); the fast suite 40 of 40 in both builds and the format check clean at every commit; goldens re-derived by their scripts (the census by its own, `golden_defaults` 60 rows). Decided by one gainer SPRT `{0, 5}` nElo at the harness regime against `778c7b0`, pre-registered in `adocs/data/S132_sprt.sh`: **H1, `Elo 16.28 +/- 8.47`, `nElo 21.21 +/- 11.01`, LLR 2.95, 3822 games in 1 h 48 m 24 s, 0 forfeits either side** (`5ba2478`, DEC-220's block gate-checked against `adocs/data/S132_sprt.log`); the claim is at least 5 nElo (DEC-063); the constants stay at their census seeds for S127 to fit with S085's caveat doubled, and the step's one question -- a second-control confirmation before they are called shipped -- is the owner's, parked in `status.md` with the coordinator's recommendation. `bench` 5066204 -> 5066204, a clock rule moving no fixed-depth node. Implemented from the step file's description (DEC-221); the published record's figures were direction only (DEC-019).
