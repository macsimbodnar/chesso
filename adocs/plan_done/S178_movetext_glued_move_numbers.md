...
done:       2026-09-07. `movetext_to_san()` splits a move number indication off the move it is glued to. **Red first at `3200a1a`**: properties 7 and 8 added to `tests/test_make_book_tools.sh` gave 8 `FAIL:` lines, every one on `game 1 cut short: cannot parse '1.e4' at ply 0`, exit 1, no book at `--out`; `castle_spaced`, the export-form control, was green from the start at its 7 entries, so the expected counts came from the twin's own output and not from memory (S174's stamp). **Green after** the three-line edit in `movetext_to_san()`: `dot == npos` drops a bare number first, then `dot > 0 && token[dot] == '.'` finds the first non-dot and either drops the indication alone (`rest == npos`) or erases the prefix, so `1.e4` gives `e4`, `2...Nc6` gives `Nc6`, `4.O-O` gives `O-O`. No `src/` file in the diff; `build/src/chesso` is `c6f0a6c3...9dc4719` and DEC-140 owes neither a `Bench:` line nor `No functional change`. `tools/gate.sh` does not exist yet (S189 is Open entry 4). **Gate**: 27/27 in `build` and 27/27 in `build-tune`, `./clang-format.sh --check` exit 0 under this machine's `CLANG_FORMAT_MAJOR=22` (DEC-146). **INV-6, identical to the recorded baseline**: depth 9 `121512 / 800769 / 62907`, depth 12 `639228 / 3430710 / 367858`, best moves `c3d5 / e2a6 / d7c8q` at both depths -- necessarily, the binary is unchanged. **Shipped book**: rebuilt from `books/8moves_v3.pgn` through the fixed tool, `cmp` identical to `src/openings.bin`, sha256 `77f47f1bd184df6d1e6be539c354b526558d2e4970c6dc565c5ea53e5db06b58`, the S175 digest; the preconditions hold, both greps print 0. **Independent oracle**: `adocs/data/S175_book_conformance.py` under `~/.venv/chess` re-derived both glued fixtures with python-chess -- `missing 0 extra 0 weight_mismatch 0 duplicate_book_entries 0`, 4 and 7 entries -- so python-chess and chesso now agree on the import form as they already did on the export form. **The boundaries were observed, not assumed**: `1.2` cuts short honestly as `2` at ply 1 (no bare-number drop on the remainder, which would be 2026-09-03_adversarial-F02's fault class); `...` alone has `dot == 0`, reaches the parser and cuts short as `...`, which is section 10's question 1 left as `accepts:` has it; `1-0` and `1/2-1/2` are still taken as results, 2 and 4 entries, the difference being the existing zero-weight drop for the losing side and not this change. **Documents**: `DEV_MANUAL.md`'s book section gains a paragraph naming both forms and the whitespace form it does not read, traced to `movetext_to_san`; `adocs/specs.md`'s book paragraph gains the S178 clause and says the digest and the binary are unchanged (section 8's third question, decided here as coordinator); `MANUAL.md` checked -- no UCI surface is touched and its book paragraph points at `DEV_MANUAL.md` for the tool, so nothing changed. **Owner questions still open**, both from section 10 and neither a bug: whether `1 . e4` (8.2.2.1's whitespace between digits and dots) is worth the same few lines or its own step, and whether the cut-short message should carry the token as written rather than the move part.
