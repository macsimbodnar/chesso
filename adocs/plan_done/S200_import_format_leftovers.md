...
done:       2026-09-07. Two edits in `tools/make_book.cpp`, nothing in `src/`. **The dots**: S178's `dot > 0` guard is dropped, so the same block runs when the first non-digit is at position 0 -- a token of dots alone is dropped, a move behind them is split off it. **The message**: `movetext_to_san()` returns `san_token_t {move, as_written}`, `as_written` taken at the token's creation before the erase; `build()` hands the first field to `algebraic_to_move()` and the second to both `report_cut_short()` messages. **Red first at `ffe207f`**, two new properties in `tests/test_make_book_tools.sh`: 13 `FAIL:` lines -- `cannot parse '.' at ply 0`, `cannot parse '.e4' at ply 0`, `cannot parse '...' at ply 1`, and `cannot parse 'Qxf7' at ply 2` failing both the as-written assertion and its bare-form negation. Green after. **Numbered 12 and 13, not the 9 and 10 this file named**: S173 landed between the writing and the doing and took 9, 10 and 11; the header comment says so. **Verified**: `1 . e4 e5 2 . Nf3 Nc6 *` and `1 .e4 e5 2 .Nf3 2 ...Nc6 *` each build 4 entries `cmp`-identical to the export-form control, `1. e4 1. ... e5 *` builds 2 with `games cut short 0`, `1.e4 e5 2.Qxf7 *` is refused naming `2.Qxf7`. **The remainder is still not re-classified**, as the traps require: `.2` cuts short as `.2` and `1.2` as `1.2`, while a bare `2` is still dropped. **Independent oracle**: `adocs/data/S175_book_conformance.py` under `~/.venv/chess` reads all three new fixtures natively and reports `missing 0 extra 0 weight_mismatch 0` on each. **Unchanged by construction and checked anyway**: the shipped `src/openings.bin` rebuilt from `books/8moves_v3.pgn` is byte-identical at `77f47f1b...db06b58` (34700 games, 0 cut short, 172232 entries), and INV-6 reproduces to the node -- depth 9 `121512 / 800769 / 62907`, depth 12 `639228 / 3430710 / 367858`, best moves `c3d5 / e2a6 / d7c8q`. **Not owed**: no pruning, `make_move`, generator or search change, so DEC-141's second tier does not apply; no SPRT and no `hyperfine`, since no `src/` file is in the diff -- which is also why DEC-140 owes neither a `Bench:` line nor `No functional change`, and `tools/gate.sh` (S189) does not exist yet. **Gate**: 27/27 in `build` and 27/27 in `build-tune`, `./clang-format.sh --check` exit 0 under `CLANG_FORMAT_MAJOR=22` (DEC-146). **Documents**: `DEV_MANUAL.md`'s move-number paragraph now says the whitespace form is read and what happens to the remainder, and its cut-short sentence says the token is quoted as written; `adocs/specs.md`'s book clause is widened rather than left at S178's wording -- the coordinator's own call, this step held the machine -- because the tool's behaviour changed and DOCS binds it to this commit; `MANUAL.md` checked, no UCI surface touched and it delegates the tool to `DEV_MANUAL.md`. **Left open, unchanged from `excludes:`**: `1 .e4` builds correctly only because S201 had already fixed the parser's leading character; the other import-format leniencies S178 listed are still untouched.
