...
done:       2026-09-24 14:34 -- one block written, three landings, two verdicts and a removal; what stays is the accumulator. **The accumulator** (`8b1bc79`): the late move reduction table holds ticks of a ply (`LMR_SCALE` 1024, a compile-time design constant beside `build_lmr_table`, form (c)), `lmr_node_adjustment`'s five terms are summed in that unit and `lmr_adjusted_reduction` rounds once at its return by `LmrRoundBias` added before the shift. **The rounding ships at 0, the parent's truncation, by measurement and not by choice (DEC-231)**: ten builds showed every bias from one tick up with the term live losing the mate in "pruning does not hide a forced mate", so the accumulator is bit-identical to the tree before it -- `bench` 4493659 with all eight `bestmove` replies identical and `tools/search_bench.py` identical at depths 9 and 12 (INV-6, DEC-215) -- and its nps cost is inside the noise floor (twelve interleaved pairs, `adocs/data/S236_nps_interleaved.txt`). **The history term** came back in that unit, `clamp(hist_sum * 1024 / LmrHistDiv, +/-LmrHistClamp)` subtracted, seeded at form (b) from a census of `quiet_history_sum` at depth 12 over the bench positions on the tree the term is added to (`adocs/data/S236_hist_census.py`, p90 367, p99 4883, 68 % of sums zero: `LmrHistDiv` 734, the clamp rule's 6812 bound by the declared top 2048) -- a first census on the rejected round-to-nearest tree was superseded and is on the record. Decided at `{0, 5}` nElo against `666b5a0`, the tree without the term, twice as DEC-231 pre-registered: at two plies of reach **H0, `Elo -1.60 +/- 4.25`, `nElo -2.05 +/- 5.44`, 15658 games in 7 h 32 m, 0 forfeits** (`3243c3f`), the interval reaching above zero and read as a walk; at one ply, DEC-231's one follow-up (`85901b3`, `bench` 5193174, +15.6 % as the census predicted), **no verdict at the harness's 40000-game limit, `Elo 2.61 +/- 2.64`, `nElo 3.38 +/- 3.40`, 19 h 08 m, 0 forfeits** (`7b47d9f`), a stalled walk near +3 read as a zero by the pre-registration's third row (DEC-063). **The term left** (`0792ef1`): its two settings, its cases, mutants W03 to W05, the pre-make read and the third argument through `lmr_adjusted_reduction` and `lmr_depth_of`; the accumulator stayed with W01 (equivalent at bias 0) and W02 and W06; `src/` against `666b5a0` is the accumulator and comments, proved by the bench signature and `search_bench` identity. Three mined goldens moved with the term's trees and were re-derived by their own scripts each time, then restored byte for byte at the removal with the term's rows kept in their GOLDEN blocks as history (DEC-142 in both directions): S095's mate row, S097's multicut mate row -- whose old row had stopped separating on the first term tree, E21 surviving until the re-mine, and killed again by the restored row at the removal -- and `test_mate_carry`'s `C_mate7_depth11` ceiling (0 -> 2 -> 0 by `S203_case_sweep.sh --ceilings` over the tracked grids). Seven older mutant anchors the new unit moved were re-cut and re-killed; the whole list validates. Cold fast checks before each landing (three) and before the removal's proofs found nothing in the arithmetic and closed findings of record each time. Debug self-play 8 games with 0 `Assertion` and `tools/gate_extra.sh` 5 stages green on every landing and on the removal (1098 s) (DEC-141); the fast suite 40 of 40 in both builds and the format check clean at every commit. The two settings' census and the readings the flip was decided on are DEC-231's; DEC-213 stands: the history-scaled reduction has read a loss twice as whole plies and a zero twice as fractions. `bench` 4493659 -> 6858745 -> 5193174 -> 4493659. Implemented from the study's description and the step file's (DEC-221); no other project's code was opened; the record's +9.09 was direction only (DEC-019).
...
**Second tier on the removal `0792ef1`, 2026-09-24 (coordinator):** Debug
self-play four rounds at 4+0.04, concurrency 8, `-log level=trace engine=true`
-- **8 games, 0 `Assertion`, 0 `disconnect`**, 107058 trace lines with 925
`bestmove` lines (`.tuning/coord/s236_removal_debug_selfplay/`), 14:15;
`tools/gate_extra.sh` on `0792ef1`: **`GATE-EXTRA-DONE 5 stages 1098 s`**
(14:15 to 14:34, `.tuning/gate_extra_2026-09-24_s236rm/`), prose, citations,
debug, sanitize and perft green. W06 kept at the agent's judgement, confirmed:
it guards the accumulator's scaling of a node term, live code with a live
killer. The step completes on the coordinator's stamp below the header.
