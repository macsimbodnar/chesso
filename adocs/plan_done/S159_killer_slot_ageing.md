...
paused_by:
author:
done:       2026-09-09. **The ageing reading of S149's -11 Elo is refuted by its own census, before a game was played, and no SPRT was spent on it (DEC-160).** Counted over 18166063 nodes on 11 recorded positions, on HEAD and on HEAD with S149's guard re-applied to an instrumented copy: the stale share of distinct second-killer offers is **1.96 % against 1.91 %, flat**. The guard did not preserve proportionally staler killers -- it roughly doubled how often a distinct second killer is offered at all, **45.4 % of nodes to 88.8 %**, and the stale count rose only with it. On HEAD the unguarded shift is already the ageing mechanism, discarding slot 1 on **72.0 % of stores**, so candidate A had **0.89 % of nodes** to act on. Duplicate rates reproduce S149's on the new set -- 72.0 % of stores and 44.0 % of nodes against 66.0 % and 44.4 % -- and the guard removes the duplication completely, 0 of 9039916, as S149 measured. **Candidate B is neutral as the file predicted**: `python3 tools/search_bench.py` at depth 9 (121530 / 801481 / 72924, best `c3d5` / `e2a6` / `d7c8q`) and depth 12 (636677 / 3520847 / 494098, same best moves) identical to HEAD to the node, its only observable being `"a search fills the ordering tables"` going red on `killers_1_duplicated > 0` for a change that alters no game -- the fence was not rewritten for it. **Candidate A was built, tested and reverted**: `killers_clear` in `src/search.cpp` declared beside `search()`, called first in the depth loop of `src/chesso.cpp` `iterative_deepening_search`, with a guard test carrying its precondition and observed red under two mutants (empty body, 3 slots left; row-0-only, 1 left) -- the second is why the test seeds all `2 * MAX_PLY` slots rather than trusting a depth 4 search to fill row 1. **S149's driver turned out not to be reproducible** -- its 11 positions are named in two documents and their FENs are recorded in neither, so 66.0 % / 44.4 % cannot be re-derived by anyone. S159's set, driver, instrumentation patch and outputs are recorded under `adocs/data/S159_*`. **The step also found a bug and did not carry it**: candidate A turned `test_mate_carry` red against a green HEAD, the S203 sweep over both sides showed `C_mate7_depth11` reporting mate lines in **one cell of nine** at HEAD with its pinned budget being that cell, and that is S204 and DEC-161. No `src/` change ships, so no `Bench:` line: `git diff --stat -- src/` is empty against `c982f9d`. 30 of 31 fast tests were green on the candidate and 31 of 31 are green on what ships, both builds
