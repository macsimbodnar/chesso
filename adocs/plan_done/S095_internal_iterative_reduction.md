...
done:       2026-09-21 01:50 -- one term in `lmr_node_adjustment` beside S098 verdict 2's four, `LmrNoTtMove` in `src/search_params.hpp` with range 0..2, off value 0 and seed 1 at form (c); at the off value the tree is the parent's exactly, proved on the tree and not assumed from the range's end (DEC-215): the tune build at `LmrNoTtMove` 0 and a Release rebuild with the default forced to 0 both bench 4646334 with the eight `bestmove` replies identical to the reference binary's; the term applies only where the entry carries no move, with the site-level case establishing the precondition inside each drive (an entry with a move gets no extra ply, and the drive fails if its planted entry does not read back); a mined mate row (`4brbr/p2p1p1p/P2P1P1P/6R1/8/K7/8/1k6 w - - 0 1` at depth 11, from this project's own S145 set) added to "pruning does not hide a forced mate" and observed red with the term unguarded; mutants J01 to J03 killed 3 of 3 by `tools/mutation_check.py`, the six S091 mutants re-verified 6 of 6 after the capture rows lost their separation; Debug self-play 8 games with 0 `Assertion` and `tools/gate_extra.sh` 5 stages green on the landing (DEC-141); the fast suite 40 of 40 in both builds and the format check clean at every commit; goldens re-derived by their own scripts (`capture_mates` depths 9, 9, 10, 10; `golden_defaults` 51 rows; the mate-carry ceilings E 11 and B 15 off a fourth recorded grid, DEC-225). Decided by one gainer SPRT `{0, 5}` nElo at the harness regime against `b25452a`, pre-registered in `adocs/data/S095_sprt.sh`: **H1, `Elo 5.92 +/- 4.50`, `nElo 7.51 +/- 5.70`, LLR 2.96, 14252 games in 6 h 39 m 58 s, 0 forfeits** (`59dadf2`, DEC-220's block gate-checked against `adocs/data/S095_sprt.log`); the claim is at least 5 nElo (DEC-063), the term stays at its seed for S127 to fit, and the node-level cut the step was first written for is not taken up. `bench` 4646334 -> 4579468. Fast checks on the landing and the record earned their keep (DEC-225 written before the ceiling raise landed; three stale attributions and a stale count corrected). Implemented from the step file's description (DEC-221); the study's pre-2026 sections stand as record.
