...
**Anonymised 2026-09-19 under DEC-221.** The engine this study read is not
named in the tree: its name, repository path, HEAD and every commit hash or
pull-request number of that repository were removed from this report and
from the study; dates and commit subjects identify each commit in that
clone's log, and the key is in the uncommitted `.moltke.local.md`.
...
Two reviewers. **The coordinator** re-derived every the reference engine figure the study
quotes from the local clone at with a parser written for this review
...
read-only, the reference engine source closed, commit log and README open) attacked the
document's inference, omissions and rule compliance in parallel; every one of
its citations was then re-verified by the coordinator before it entered this
report. The findings below are the merged, re-ranked set; the last section
records the cold pass finding by finding with the coordinator's verdict.

Evidence standard: every figure re-derived by a command, every chesso claim at
`file:line`, every the reference engine claim at a commit hash. Nothing outside this file
was written. No match, fit or timing was started. The the reference engine source was
opened only for the four greps that confirm §3.3's structural claims (killers
and countermoves absent at HEAD; `parameters.rs` a two-mode macro with no
declared tunables at HEAD; the continuation tables carrying an in-check and a
noisy axis); one grep returned tuned integers, which are reproduced nowhere.

## Reproduction of the ledger

| quantity | study | this review | note |
|---|---|---|---|
| commits with an SPRT block | 796 of 1805 | 796 of 1805 | exact |
...
| non-regression runs | "about a quarter" | 408 of 1064 runs, 16.46M of 41.2M games, mean 40,347 games | F08 |
| removal-type commits | 241 | 240 subjects, 158 with a block | fine |
| lines of source | ~11k | 17.9k | cosmetic |

Every individual Elo/game figure in §3.1, §3.2, §4, §5 and Appendix A that
was checked (about seventy) is in the clone at the commit and control stated,
with the three mislabelled controls of F15. The licence chronology is exact:
MIT 2023-04-16, deleted 2025-02-22, AGPL-3.0 2026-01-26. The README rating rows (v0.7.0 3500, v0.8.0 3765) are in
`README.md` at HEAD. The chesso claims — `build_lmr_table()` at
...
### 2026-09-19_study_review-F01 high the reference engine's game counts do not price chesso's verdicts, and the study's operating rule says they do

Status: open

Lines 137-142 are the load-bearing claim: rank by the STC Elo the reference engine
...
cheaper to measure than a small one, and the reference engine's ledger is the best
...
not a new question."** the reference engine is a network engine at 3765 (line 207): a
further above-band source, which is evidence *for* the placement rule as the
owner stated it. `grep -n "S181\|DEC-176"` over the study returns nothing.
The same paragraph's superlative — "the highest measured Elo per machine-hour
anywhere in this report" — is false by the study's own table: time management
by node distribution (+17.59 over 2,352, ) and maximum time by move
number (+24.01 over 1,870, ) are 16 to 28 Elo per thousand games
against the correction family's 4 to 10, S132 owns both, and it sits at
position 12 unpromoted.
...
### 2026-09-19_study_review-F03 medium §3.2's mechanism descriptions were derived by reading AGPL source, and neither the study nor the rules say what that implies for the implementer

Status: open

Lines 51-54 record reading `src/search.rs`, `src/movepick.rs`,
`src/history.rs`, `src/transposition.rs`, `src/time.rs`, `src/nnue.rs`,
`src/board.rs`, `src/parameters.rs`; lines 18-24 conclude that AGPL does not
consent and that the compliant route is "from first principles or from a
published description". An implementer following N1, N2, N5 or N10 implements
from a source-derived specification: N10 (line 232) names three scaling inputs,
N1 (line 223) the exact bound condition and which value feeds which consumer.
Reading is not banned and never was (DEC-014, DEC-016: ideas are used freely),
and the descriptions are prose, not code (see the rules check). But the house
standard is one directory over — S181's stamp certifies *"No Lynx source file,
evaluation table or test data was read (DEC-016)"* — and nothing in AGENTS.md
says who may implement what a source-reading agent described.

What it changes: proposed decision 1 (§7) should carry a clean-room clause:
every §3.2 row is labelled "read from AGPL source, described in prose"; a step
drawn from it is implemented by a subagent whose brief carries the prose and
forbids opening the clone `.moltke.local.md` names; the coordinator states in the
stamp that it did. Ordinary clean-room practice, and AGPL is where it matters
most.
...
Line 363: the reference engine's "+37.65 skip bad noisy in qsearch" and "+14.08 split
noisy by SEE" were "measured *without capture history existing*". The clone
says the opposite: noisy history replaced LVA in (2025-02-22),
the SEE split landed in (2025-02-23), the qsearch skip in
(2025-02-25). Line 361 attributes the "+29.79 fully staged
move picker" (, 2025-03-27) to deferring losing captures behind the
quiets; the SEE split had existed for a month, so the deferral cannot be what
bought, and the commit body carries no prose. And 2026-01-30
"Simplify away bad noisy qsearch pruning" passed non-regression at +0.52 ±
1.51 over 51,834 STC and −0.00 ± 1.16 over 77,828 LTC — the +37.65 rule is
gone at HEAD, and the study quotes the figure three times (lines 187, 363,
506) without saying so.

What it changes: row 3's conclusion — S025 stays gated on S023 — survives and
is strengthened: their bad-capture gains were measured *with* capture history
present. Row 1's explanation should be struck; the ledger cannot say what the
staging bought. The +37.65 is a 2025 number for a rule the reference engine no longer has.

### 2026-09-19_study_review-F05 medium The trap table omits that the reference engine replaced internal iterative reduction, chesso's next open step
...
is real in the reference engine's record; its *form* was decided there by measurement, and
...
constant quoted in another engine's commit message is that engine's constant
and reseeded seven files (S095, S097, S098, S109, S113, S114, S132); S099 was
...
"SMP SPSA session" (,,,,,
), verified at multiple threads — the arena §3.4 rules out; seven
carry an STC run beside the LTC one and **five of those STC runs are
negative** (−1.64, −1.49, −6.87, −6.52
, −1.43 ); the +73 is the sum of the LTC pass of each
session. The titles state 28k to 200k *tuning* games on top of the verification
— 13 to 91 hours at chesso's throughput, several at 40+0.40. Chesso's verdicts
are at 8+0.08 only (DEC-083), its longer-control instrument is a fixed
1000-pair estimate at 545 games an hour (DEC-202), and its one shipped SPSA,
S085, was tuned at 2+0.02 and verified once at 8+0.08 — the same gap in
miniature. Chesso's own lanes (S222 8 h 37 m, S231 estimated 8 h 45 m) are the
right size for a night; the reference engine's sessions are not the model for one.
...
Line 340, repeated in Tier-4 item 14 (lines 441-445). the reference engine's 408
...
their budget" is a quarter of a distributed instance's budget, and the
deleting runs are its long ones.
...
All verified in the clone at simplification bounds. N5 hindsight: 2025-07-11 (+0.99 over 33,724) and 2026-02-20 "Simplify away
hindsight reductions for FDS" (+0.38 over 58,266) — the mechanism survives at
HEAD in reduced form. N11: "Update quiet history on TT hit" (+3.51
...
simplified by, and (the last +0.06 over 72,652).
None of these is a removal of the technique except the PCM-on-TT-cut cousin;
all say that the form to build is HEAD's, not the 2025 landing's, and the
study describes the landings.

What it changes: every §3.2 row that becomes a step describes the mechanism as
it stands at, and §4 gets these rows.
...
### 2026-09-19_study_review-F11 medium Tier-1 item 3's pair for S188 would ship an extension on a zero, and the the reference engine datum's form is unknown
...
said to have measured as free to remove. Line 334 calls the reference engine's removal
...
not say which form: The reference engine moved its extension inside the loop in
...
worst-case walk, and the the reference engine datum as "form unknown" (P3).
...
### 2026-09-19_study_review-F13 low Tier-3 item 11 has no the reference engine price, and the accumulator alone tests rounding only

Status: open

Lines 259-269 and 428-432. the reference engine's fractional LMR predates its ledger:
"Fractional LMR" 2024-03-19 and "Fully fractional LMR"
2024-08-17 carry a `Bench:` line and nothing else; 2025-11-26 then
...
### 2026-09-19_study_review-F14 low Four phrasings gesture at a the reference engine scale where the study promises shape only
...
is a tuned integer and none is offered as a seed, so the study's line 33 ("no
constant from the reference engine appears") holds on its own terms; but each came from
source and each is the kind of hint DEC-134's reasoning is about — a seeded
vector converges near its seed. The cold reviewer rates this medium; the
coordinator rates it low because an implementer still has to fit every one of
them and the numbers named are units and ranges, not values.
...
LTC figure of ; STC read +1.81 ± 1.46 over 57,166. Line 338: "+4.15"
...
**Licence and DEC-104.** Chronology exact; reading right: AGPL-3.0 does not
consent to adaptation into an MIT tree, so DEC-104's adaptation clause is shut
for the reference engine — and moot, as the study says, because COPYING bans copying in
every form. Proposed decision 1 (§7) is sound and should gain F03's clean-room
clause.
...
study's statement at lines 33-34 **holds**, with F14's rewording owed.
...
another engine's commit messages are facts about that engine's tests; DEC-019
governs their use and the study cites it in the right places — and then
carves variance out of it, which is F01.

## Plan proposals

Each is an option set with a recommendation; the owner decides, one item at a
time. Prices are DEC-143's, not the reference engine's (F01).
...
S111 in DEC-134's forms (one document step). The reference engine adds two things to the
...
The reference engine is a 3765 network engine and so more above-band evidence, not less;
(c) leave as is. **Recommend (a)**: it costs one night and one document step,
changes no owner decision, and its verdict is what (b) would need anyway.
...
decision; (c) both forms, two verdicts. **Recommend (b)**: the form the reference engine
...
unstated, network engine) and the reference engine's (form unstated), the worst-case walk
...
management by node share) owns the two best Elo-per-game rows in the reference engine's
...
sites later one at a time); N5 (hindsight reductions, in HEAD's reduced form
per F09; the parent's reduction is a first-class quantity after S098); N4
(cutoff count). N7 (threat-indexed history) waits for a threat map that S101
also wants — one step builds the map and prices it, then both consume it. N8
waits for S023/S025 (F17). The history-update family (N9–N12) is four
candidates, not a DEC-082 block (F12). Every seed in DEC-134's forms; every
brief under F03's clean-room clause. Placement: after P1's probe and P5,
before S112. Price: DEC-143's, 12 to 20 hours each worst case.
...
step, because each wants a fraction of a ply. No the reference engine price exists for it;
it is a hypothesis test about a recorded chesso zero (DEC-213), and the block
design is what lets it discriminate.

**P6 — A simplification lane, priced at a night per removal (F08).**
Candidates: the killer slots once S231's fitted stack is in (the reference engine +0.50 on
removal; S149 and S159 already stopped investing); the countermove table
(subsumed by continuation history in the reference engine's record, no number); delta
...
the reference engine's 28k-to-200k-game sessions. Pin the verification: an `{0, 5}` SPRT
...
8+0.08 — five of the reference engine's seven dual-control sessions did. Options: (a) as
...
The owner's idea, read as the reference engine's practice: every commit that carries a
measured change also carries the harness's result block — Elo with interval,
control, threads, hash, LLR with bounds, games with W/L/D, pentanomial — so
`git log` is the measurement ledger. Three readings were considered; the
first is the one recommended.

**(A) The verdict block in the commit.** The reference engine can put it in the *landing*
...
script — what this study did to the reference engine and chesso cannot do to itself; the
...
ledger from acquiring the passes-only bias the study had to work around in
the reference engine's (§1.1). What it costs: a format (below), a gate clause of about
...
book, forfeits and wall time, which chesso records and the reference engine does not.
...
**(B) An STC and an LTC verdict per change.** The reference engine does it on donated
hardware, and even so only 218 of 796 commits carry both. Chesso's longer
control runs at 545 games an hour (S151) against about 2,130 at 8+0.08; a
per-change LTC verdict would quadruple the ledger's mean. DEC-202's fixed
...
- No the reference engine constant is reproduced in this report; the one grep that
...
|---|---|---|---|
| 1 | the reference engine game counts do not price chesso verdicts; DEC-019 covers variance; `plan.md`'s transfer ratios | **confirmed**, promoted to the top | F01 |
| 2 | §6 item 1 misreads DEC-133; DEC-176 (c) ruled; S132 is the better-priced promotion | **confirmed** (DEC-133 Rejected line, DEC-176 (c) read;, in the clone) | F02, P1, P4 |
| 3 | §5 chronology reversed; removed the +37.65 rule | **confirmed**, merges with the coordinator's own finding | F04 |
...
| 7 | §3.2 derived from AGPL source; provenance unstated | **confirmed as medium**, not medium-high: reading is allowed by DEC-014/016; what is missing is the label and the clean-room clause | F03 |
| 8 | Later simplifications on N5, N11, N10, N8, qsearch LMP/futility | **confirmed**, all eight hashes and figures match; wording tightened — most are simplifications of the form, not removals | F09 |
| 9 | the reference engine's check-extension removal is of unknown form (, ) | **confirmed**; merged with the coordinator's pair finding | F11, P3 |
| 10 | DEC-082 misapplied to N9–N12 | **confirmed** | F12 |
| 11 | Fixed-point LMR has no the reference engine price; accumulator alone tests rounding | **confirmed** on the price and the chronology (,, ); the "cannot discriminate" half is answered by the block-and-bisect design | F13, P5 |
