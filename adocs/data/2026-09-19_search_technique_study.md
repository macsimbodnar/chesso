# A reference engine, read for ideas — 2026-09-19

What a strong open-source reference engine does that chesso does not, what its own
measurement record says each of those things was worth, and what it added and
then threw away. Written to improve chesso's plan, not to import its code.
The engine is not named here: see §0 and DEC-221.

---

## 0. The licence, first, because it governs everything below

**The reference engine is AGPL-3.0 since 2026-01-26.** It was MIT before that
(a licence file created in 2023-04), then unlicensed from 2025-02-22 until the
AGPL file landed. The clone this study read is at a HEAD well inside the AGPL
era. Its name, path, HEAD and the commits behind every figure are recorded in
`.moltke.local.md` and nowhere in the tree (DEC-221).

This matters in a specific way. DEC-104 permits *adaptation* — never
copy-paste — from an open-source engine "where their licence consents". **AGPL
does not consent**: it is copyleft, stronger than GPL, and any derivative work
must ship under it. So the adaptation door DEC-104 leaves open for permissive
engines is **shut for the reference engine**, and the only lawful and rule-compliant use is
the one the project already makes of every engine: read the *technique*,
implement it here from first principles or from a published description.

The COPYING rule is stricter than the licence anyway — "nothing is copied,
ever" — so in practice nothing changes. But a future agent must not read
DEC-104's "adapted, never copy-pasted" clause and reach for this repository.
**This is proposed as a decision** (see §7).

**What this report contains and does not contain.** Technique names,
mechanisms described in prose, and Elo measurements read out of commit
messages. **No source, no table, and no constant from the reference engine appears
anywhere below.** Every tuned integer in their search was deliberately left
out: under DEC-084 as amended by DEC-105 another engine's tuned output is never
a seed, and the reference engine's constants are SPSA output written straight into the
...
Two sources, both read on 2026-09-19.

1. **The local clone**, at the HEAD `.moltke.local.md` records: 1805 commits.
   Read: the search, move-picker, history, transposition, time-management and
   network source files, parts of the board and parameter files, the README
   and the licence. **Read for technique, described in prose; under DEC-221
   no agent that implements a row of §3.2 opens this clone.**
2. **The commit log as a measurement ledger.** The reference engine commits its SPRT
   results into the commit message: Elo with a 95 % interval, time control,
   threads, hash, LLR with its bounds, game count, pentanomial, and a link to
   its OpenBench instance. **796 of 1805 commits carry at least one such
   result; 794 of them are dated 2025-02 or later.** They were parsed into
   `2026-09-19_technique_ledger.py`, committed beside this file after the
   2026-09-19 review; every figure quoted below came out of that parse and is
   re-derived by running it over the clone `.moltke.local.md` names.

Web sources fetched: its README rating table, its progress-tracking wiki page
(the STC/LTC regime) and its latest release notes.
...
- **It is passes only.** OpenBench keeps failed tests on the instance, not in
  ...
  floor.**
- **Their bounds are stated in logistic Elo; chesso's are in nElo.** One commit
  in 1805 mentions nElo at all. chesso's DEC-143 pre-registers with
  `model=normalized`, so the pairs are on different scales — but `README.md`
  in this directory already records the conversion this project measured:
  **5 nElo was 3.54 logistic Elo at S165's 44.75 % draws, and 3.87 to 3.99 at
  the lower draw rates of S021, S076 and S107.** So chesso's `{0, 5}` is
  roughly the reference engine's `[0, 3.5]` to `[0, 4]`, which is where most of their
  testing sits. **The two regimes are close enough that their run lengths are
  informative here**, and the reported Elo figures are the same kind of number
  as chesso's ledger reports. Bounds they used, by frequency: `[0, 3]` 372
  times, `[-2.75, 0.25]` 215, `[0, 4]` 169, `[-3, 0]` 103, `[0, 5]` 89. The
  2025-02/03 rebuild era — the part most relevant to chesso — ran mostly at
  `[0, 5]` logistic, which is *looser* than chesso's gainer pair, so its run
  lengths are if anything optimistic.
- **The rebuild base already had an NNUE.** The February 2025 sequence is a
  ...
  evaluation, not a rewrite of the engine. Every figure in that ladder was
...
## 2. The scale of their programme, and what chesso can afford of it
...
24/7**, for the passes alone. This engine
runs this on a distributed OpenBench instance with donated hardware. chesso has
one machine and the MACHINE rule.

So the reference engine's record cannot be replayed. It can only be **mined for ordering**,
and the mining rule falls straight out of their own data.
...
by `plan.md`'s own admission. This engine's ledger is the best available
instrument for finding the remaining items that are *not* slow class. Every
recommendation in §6 is ranked by the STC Elo it measured, ~~because that
figure is a prediction of **what the verdict will cost**, which transfers far
better than the Elo itself (DEC-019 is about the Elo, not about the variance)~~.
...
as that engine's, never as this project's.**

### 2.2 Their gains per month, which is the shape of the wall
...
## 3. What the reference engine has that chesso does not
...
without a number. Three columns: the mechanism, what the reference engine measured for it,
and where chesso stands.

### 3.1 In chesso's plan already — the reference engine's number is ordering evidence

| Technique | reference STC Elo (games) | chesso |
|---|---|---|
...
| Internal iterative reduction | **+5.11** (10202) — **replaced 2026-02-15 by a reduction term inside LMR**, a simplification at −0.32 ± 0.78 over 195,882 games; the node-level cut does not exist at HEAD (review F05) | S095, re-formed as that LMR term, DEC-222 |
...
+9.17, +3.77 and +6.15** in the reference engine, at **3,000 to 8,300 games each**. That is
...
above ~3100'"). **The reference engine does not settle that band either, and this study
did not try to**: its absolute strength in March 2025 is not derivable from
the record — the rebuild was a bare search on top of an existing network, and
no rating was published between its 2024-08 release (3500) and its 2025-08
release (3765). What the ledger does establish is the effect size inside an engine
...
Ranked by the STC Elo the reference engine measured, which is also the inverse of what the
verdict costs.

| # | Technique, in prose | reference STC Elo (games) |
|---|---|---|
...
| N24 | **Time management: hard bound by a constant scale; soft bound ramping with the move number; best-move instability as a factor; score trend as a factor.** Their soft limit is a *product of five* multipliers — node share, score trend, PV stability, eval stability, best-move changes — not a sum of adjustments. | max time by move number **+24.01** (1870); constant hard scale +8.19 (7808); eval stability +6.15 (7744); PV stability +3.75; best-move instability +4.00 LTC; score trend +4.34 LTC |
...
**The reference engine accumulates the reduction in fixed point — at sub-ply resolution —
and divides once at the end. chesso accumulates it in whole plies.**
...
the tree (DEC-213). The reference engine measured the same idea at **+9.09** (4970 games).
The three differences that could explain a sign flip are: their history sum is
richer (quiet + two continuation tables, threat-indexed); their reduction is
fixed point, so the history contribution is a fraction of a ply; and their
...
- **Their continuation tables are bucketed by `(in check, move was noisy)`** as
  well as by (piece, to) × (piece, to). chesso's `cont_hist` and `cont_hist2`
  have the two piece-square axes only. Two extra bits on an existing table.
- **They carry a pointer to the continuation sub-table in the search stack**,
  ...
  side at S231 — no per-ply stack, pass the key down — for the same reason
  (S191 measured 1.49 % of nps for one extra read per node). Worth knowing the
  alternative exists if a third continuation table is ever wanted.
- **They have no killer moves and no countermove table.** Both were removed
  (§4). Ordering is history-only, plus the geometry terms of N15.

### 3.4 Not applicable to chesso, recorded so nobody re-derives it

- **The NNUE.** Their eval is a network and nothing else — there is no
  hand-crafted evaluation in the tree at all, and **no commit in 794 prices a
  ...
  into a threat — all touch its move-picker source: they are move-ordering terms,
...
- **SMP, NUMA, thread voting, MultiPV, large pages.** chesso is
  single-threaded by DEC-089 and the target is the CCRL 1CPU list. Their
...
### 3.5 What their SPSA cadence says
...
weeks, never once and for all. Their tuning machinery has the same shape as
chesso's `CHESSO_TUNE=ON` dual build: a macro in its parameter module that
...
(DEC-118) is the normal answer. At HEAD their declared set is *empty*: the
tuned values have been folded back into the search source as literals, and a
session re-declares whatever it wants to move.
...
it is a recurring night**, and it is the single largest line item in their
...
## 4. The traps: what they added, measured, and later removed

241 commits begin with Revert, Simplify, Remove or Drop. Each removal that
carries a number is an SPRT showing the engine was **no worse without the
...
## 5. Where the reference engine's numbers disagree with chesso's own measurements
...
| chesso measured | the reference engine measured | Candidate explanation |
|---|---|---|
| **Staged move generation: 0** (S006, DEC-004 calls it "the largest wrong estimate in the project") | **Fully staged move picker: +29.79** (1356 games) | Their staging gain is not the movegen saving. ~~It is that a fully staged picker **defers losing captures behind the quiets**~~ **Struck 2026-09-19 (review F04): the SEE split of noisy moves had existed for a month when the staged picker landed, so the deferral is not what it bought, and its message says nothing.** The candidate that survives is that a staged picker never generates quiets at a node that cuts off on the TT move or a good capture. chesso is staged but does not defer bad captures (S025, reserve). The saving chesso measured and whatever this engine measured are different changes wearing one name. |
| **SEE pruning in quiescence: 0** (S015) | **+5.08** (11424 games) | Their threshold is not zero and not fixed: it moves with the distance from alpha, the corrplexity, and the move's own history. chesso's is a fixed threshold. A zero on the fixed form does not price the adaptive form. |
| **Bad-capture ordering: slower, three ways** (S025, DEC-004) | **Skip bad noisy in qsearch: +37.65**; **split noisy by SEE: +14.08** | ~~Both of theirs were measured *without capture history existing*~~ **Corrected 2026-09-19 (review F04): both were measured *with* capture history present — noisy history replaced LVA on 2025-02-22, the SEE split landed on 2025-02-23 and the qsearch skip on 2025-02-25** — and with a SEE call inside the picker's own loop rather than a pre-pass. That is direct support for S025's gate on S023 (capture history first), not a disagreement to explain. The +37.65 rule was itself removed as free on 2026-01-30, +0.52 ± 1.51 over 51,834 STC and −0.00 ± 1.16 over 77,828 LTC. |
...
check A12). The reference engine has the same mechanism and a **generalised** version of it
...
1. **Promote the correction-history family out of reserve.** S099 → S110 →
   S111, plus a new step for "correction applied in quiescence" (their +13.85)
   and one for corrplexity as a margin and reduction input (their +9.17,
   ...
   check, and this engine does not settle the band either way (§3.1). ~~What it
   ...
   2026-09-11 after S181's re-banding. A 3765-rated network engine is more
   above-band evidence. What DEC-133 already permits is the path taken: S099
   runs as the probe on a spare night, after S232 reseeds it (DEC-222).**
...
3. **Re-price S188 (check extensions) before running it.** Three engines have
   ...
   **Corrected 2026-09-19 (review F11): a `{-5, 0}` pair accepts H1 on a truth
   ...
   its form: the engine's removal message names neither, and its 2024 history
   moved the extension inside the loop and back out within a fortnight.**

### Tier 2 — new steps, cheap verdicts, no plan step exists

Ranked by the reference engine's STC Elo. Each is one step.
...
13. **Schedule SPSA as a recurring night, not as S127.** Fourteen sessions,
    ...
    supports.
14. **Budget a simplification lane.** A quarter of the reference engine's budget goes to
    removing things that stopped paying, and four of chesso's shipped rules
    are candidates on the reference engine's own removal record: the killer slots, the
    countermove table, check extensions if S188 ever lands, and whatever of
    S109's block the later pruning work makes redundant.
15. **Take an LTC reading on the items where the reference engine's STC and LTC disagree.**
    Their split is informative: correction history, singular extensions and
...
1. **The reference engine is AGPL-3.0 and is read for technique only.** DEC-104's
   adaptation clause does not reach it, because its licence does not consent.
   No source, no table and no constant from it enters this repository, and any
   technique taken from this report is implemented from first principles or
   from a published description, with its constants fitted here. (§0)
   **Taken 2026-09-19, with a clean-room clause and anonymised in the tree
   (DEC-221).**
...
Every feature commit of the reference engine's search rewrite, in the order it landed, with
the Elo it reported at the control stated. This is the closest thing in public
to a controlled ordering of what each classical search technique is worth when
added one at a time to a bare search. Read it as **ordering and as a cost
estimate**, never as a prediction of a chesso verdict (DEC-019). All figures
8.0+0.08 unless noted.
...
```
git -C <clone> log --format='%H%x01%ad%x01%s%x01%b%x02' --date=short
```
...
whose time control parsed as `8.0+0.08`. The HEAD this was read at, and the
clone's identity, are recorded in `.moltke.local.md` and nowhere in the tree
(DEC-221). The parse is `2026-09-19_technique_ledger.py` beside this file; run
it as `python3 adocs/data/2026-09-19_technique_ledger.py <clone>`.
...
**The engine is not named in this file (DEC-221):** the key — name, clone
path, HEAD, licence commits and the commit behind every figure — lives in
`.moltke.local.md`; dates and commit subjects identify each commit in that
clone's log; `2026-09-19_technique_ledger.py` re-derives every aggregate.

- **F01, high — the cost rule is withdrawn.** §2.1's operating conclusion and
  every hour figure in §6 read that engine's game counts as chesso's verdict
...
- **F02, high — §6 item 1 and §7 item 2 misread the reserve.** DEC-133's
  ...
  and this 3765-rated network engine is more of the same. Not taken; S099 runs
  as DEC-133's probe after S232's reseed. Time management (S132), not the
  correction family, has the best Elo-per-game rows here, and moves up.
- **F03, medium — provenance.** §1 now says the rows of §3.2 were read from
  AGPL source and described in prose, and that no implementer of one opens
  the clone (DEC-221's clean-room clause).
...
- **F09, medium — later simplifications** on N5, N8, N10, N11 and quiescence
  pruning are not listed in §4; the step files S234 to S238 build HEAD's form.
...
- **F13, low — Tier 3 item 11** has no price in that ledger (its fractional
  LMR predates the ledger); it is S236, a hypothesis test in one block.
