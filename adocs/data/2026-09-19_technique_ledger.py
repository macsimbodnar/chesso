#!/usr/bin/env python3
"""Read an engine's commit log as an SPRT ledger, 2026-09-19.

Some engines commit their harness result block into the message of every
merged change -- an OpenBench-style block of `Elo | x +- y`, `SPRT | <tc>s
Threads=n Hash=nMB`, `LLR | l (a, b) [lo, hi]`, `Games | N: n W: w L: l D: d`
and `Penta | [...]` lines, one block per control. That log is a ledger of
...
re-derived by a command and not re-read from a transcript.
    ...
    python3 adocs/data/2026-09-19_technique_ledger.py <clone> [out.json]

The clone's identity is deliberately not in this file (DEC-221); the one the
study read is named in `.moltke.local.md`. Pairing is positional per commit:
the i-th Elo line goes with the i-th SPRT, LLR and Games line of the same
message, and a commit whose counts disagree keeps the shorter pairing.
"""
...
def parse(clone):
    log = subprocess.run(['git', '-C', clone, 'log', '--date=short',
