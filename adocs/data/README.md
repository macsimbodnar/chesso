...
| `S197_script_mutants.py` | the twelve cuts to `tools/gate_extra.sh` that `tests/test_gate_extra_script.sh` was observed red under, S197, 2026-09-10: **12 of 12 killed**. Shell-script mutants, so deliberately not in the `tools/mutants/` registry `tools/mutation_check.py` drives -- nothing here builds the engine. Two of the seven cases exist only because a first version of the test let `M5_no_bench_compare` and `M12_no_root_check` through, which is the reason the cuts are kept rather than described. Every anchor must appear exactly once or the script refuses |
