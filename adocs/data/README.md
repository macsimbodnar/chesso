...
| `S097_v2_sprt.sh` | the pre-registration of S097's **second** verdict, the multicut, written the same day and against the first verdict's landing: the candidate is one default, `SeMultiCut` 0 to 1, plus the coverage that default makes possible -- the rule's guard case, the accepts' mined mate row and the mutants E20 to E22 -- and the file says why none of them could land with verdict 1, the switch being a compiled constant the release build folds away. It carries what this pair gets for free (the reference binary **is** the tune build at the off value), the direction a pruning rule's bench should move and what an unmoved one would mean (DEC-212, DEC-214), the same `{0, 5}` cost, and an interpretation written in advance for the reading the record predicts here: **a stalled walk near +3 is terminated, recorded as a zero and dropped** (DEC-063), because the traced +5.7 and +6.2 are from engines three hundred points above this one and one engine dropped the rule at this band |
