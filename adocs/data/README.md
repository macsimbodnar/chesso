...
| `2026-09-19_search_technique_study.md` | the 2026-09-19 study of a strong open-source engine under a copyleft licence, evaluated by a network, read for technique only and **anonymised in the tree (DEC-221: no name, path, commit hash or pull-request number of that repository; the key is in `.moltke.local.md`)**: what it does that chesso does not, priced from its own commit-message SPRT ledger -- 796 of its 1805 commits carry a full result, 794 since its 2025-02 search rewrite, 41,964,262 games over every recorded run -- an inventory of 25 techniques with no chesso step, each with the Elo and game count it measured, a removal ledger of what it added and later deleted as free, the three places its numbers meet chesso's own S006, S015 and S025 zeros, and its full 2025 rebuild ladder. **Corrected in place on 2026-09-19** after the adversarial review (`adocs/audit/2026-09-19_study_review.md`, 17 findings, two reviewers): its cost rule withdrawn -- a chesso verdict costs what DEC-143 says -- its reading of the reserve placement corrected against DEC-133 and DEC-176, its provenance labelled, its §5 chronology and §4 traps repaired; the corrections are struck through in place and listed in a dated section at its end. No source, table or constant from it is reproduced (DEC-016, DEC-084 as amended by DEC-105). DEC-220 to DEC-222 are what the review decided |
| `2026-09-19_technique_ledger.py` | the parser behind every number in that study and its review: reads `git log` of a clone whose commit messages carry OpenBench-style result blocks, pairs Elo, control, bounds and game count positionally per commit, and prints the aggregates -- commits with a block, games over every run and first-run-only, controls and bounds by frequency, the bucket table of games against reported Elo at 8+0.08, the simplification-pair median, monthly sums, SPSA and removal counts. `python3 adocs/data/2026-09-19_technique_ledger.py <clone> [out.json]`; run 2026-09-19 against the clone `.moltke.local.md` names, reproducing the study's counts exactly where they were right and the review's where they were not |
