...
- **S188's verdict is H0, 2026-09-22 12:50:17: `Elo -13.84 +/- 8.95`, `nElo
  ...
  loss and not a zero: the nElo interval [-29.15, -6.27] sits wholly below
  zero (DEC-223's fast class; DEC-194's shape). The pre-registered H0 reading
  binds: **the rule leaves the tree in the same step** -- window, gate, the
  scan's third term, the ply, three settings, ten cases, twelve mutants, the
  MANUAL rows -- the three repaired cases go back by their scripts (4 and 9,
  the equality), the removal proved by `bench` 4493659 and `search_bench`
  identity with `d819a09` at 9 and 12 (INV-6); the re-mined S097 row stays if
  a targeted E21 run kills it on the reverted tree. The mined mate set's
  146 -> 180 is a fixed-depth reading of a larger tree and not a reason to
  keep it. `Incomplete mating PV` 2 against 0, an observation. Evidence
  `adocs/data/S188_sprt.log`, `S188_sprt_pairs.txt`. The sixth verdict-closing
  commit under DEC-220. **Next, in order:** S188's agent removes the rule
  (writing, builds, the identity proofs, E21 targeted), a cold fast check
  over the removal diff, the gate and the removal commit with `Bench:
  4493659`; then **S132's confirmation launches** (DEC-229), the machine
  being free from then to its marker; S188 completes with a decision that the
  check-extension question has no successor step. Machine idle from 12:50; no
  watcher armed.
...
- **S097 is done, 2026-09-22 02:05, on a zero and an H1, and `plan_current/`
  ...
  +/- 5.13`, H1) both ship at their defaults of 1 with the six settings at
  DEC-105 (c) seeds for S127; `bench` 4579468 -> 5066204 -> 4493659. The
  ledger holds twenty-five (regenerated by `tools/ledger.py`: mean
  4 h 42 m, median 4 h 57 m, 259953 games in
  117.67 h; fast class eleven at 1 h 57 m, slow fourteen at
  6 h 51 m) and the pending count is 42 to 52 verdicts, 273 to
  342 machine-hours. `specs.md`'s search row carries both verdicts. The
  branch `s097-v2` (four WIP commits, all squashed into `63ad848`) is deleted.
...
- **S132 is done, 2026-09-21 19:45, on an H1, and `plan_current/` holds S097
  ...
  `bench` 5066204 -> 5066204 (a clock rule moves no fixed-depth node); the
  ledger holds twenty-four (regenerated by `tools/ledger.py`: mean 4 h 41 m,
  median 4 h 56 m, 249483 games in 112.72 h; fast class eleven at 1 h 57 m,
  slow thirteen at 7 h 00 m) and the pending count is 43 to 53 verdicts, 286
  to 356 machine-hours. `specs.md`'s time-management row carries the three
  factors and the verdict. The step's second-control question is the owner's
  and sits in Parked below. **Next: S097 verdict 2** -- rebase `s097-v2`
  (three WIP commits, `8232aff`) onto this completion, "machine free" to its
  agent for the verification pass (the third guard leg and E23 compiled, both
  suites, one clean E01 to E23 mutation run, `bench` 4493659 re-taken,
  `pick`/`fires` from the existing sweeps), its Report 4, the squash-landing
  with the specs passage, Debug self-play and `gate_extra`, `REF` pinned to
  the landing's parent, and its SPRT tonight. Machine idle from 19:13; no
  watcher armed.
...
- **S097 verdict 1 is landed and its gainer SPRT is running, launched
  ...
  `noob_3moves.epd`, seed 20260921045106, output
  `.tuning/sprt_s097_v1_20260921_045106`, pid 186722.** The landing
  (`88ec74f`, `bench` 4579468 -> 5066204): `negamax_at` gains a trailing
  `excluded_move`; a verification search at `(depth - 1) / 2` against a window
  `SeMarginPerDepth * depth` below the entry's score, with the entry's move
  excluded, extends that move by one ply when every alternative fails; a node
  under an exclusion takes no table cutoff, writes no entry, makes no null
  move, runs no reverse futility and starts no verification of its own, and a
  node whose only legal move was excluded returns its own alpha (DEC-226).
  Four settings at DEC-105 (c), `SeExtend` the off switch proved to the node
  against the parent (DEC-215 clause 2), `SeMultiCut` 0 shipping verdict 2's
  rule inert and proved so. Fifteen cases, eighteen mutants killed with the
  twelve older anchors the diff moved re-run. Fast check found one real
  problem before the landing -- the gate read the entry through the raw slot
  pointer after the null-move recursion, masked at today's seeds -- repaired
  by copying the fields out before any recursion. Second tier on the landing:
  Debug self-play 8 games with 0 `Assertion`; `gate_extra` 5 stages green.
  The node-explosion check: fixed-node depths 17/13/17 -> 16/13/15 on the
  three `search_bench` positions, the cost the SPRT has to pay back, recorded.
  Watcher armed with four exits, hourly progress, a 40 h ceiling; worst case
  41861 games (19.8 h at 2110 an hour), 25591 on a bound (12.1 h). **When the
  marker fires:** the verdict-closing commit with DEC-220's block (subject
  "Record S097 v1's <verdict> for the singular extension"); **H1** keeps the
  extension and the step's agent lands verdict 2 (the `SeMultiCut` flip with
  its guard case, mate row and mutants) for its own SPRT; **H0** flips
...
- **S095 is done, 2026-09-21, on an H1, and `plan_current/` is empty.** The
  ...
  `bench` 4646334 -> 4579468; the ledger holds twenty-two (regenerated by
  `tools/ledger.py`: mean 4 h 36 m, median 4 h 56 m, 225581 games in 101.47
  h; slow class twelve at 6 h 48 m) and the pending count is 45 to 55
  verdicts. `specs.md`'s search row carries the S095 passage with the
  verdict. **Next: S097** -- its agent's code-writing phase is done and
  stashed (`git stash list`, "S097 in progress"): the verification search
  with its gates, the singular extension (V1), the multicut inert behind
  `SeMultiCut` 0 (V2), 13 guard cases, 17 mutants and two pre-registrations;
  its first compile in the S095 record's gate run showed one red case of its
  own ("the excluded move is the one move the node does not search",
  `REQUIRE_EQ` 1 against 15 -- a cutoff after the first move, most likely),
  to be fixed in its verification phase. Pop the stash, resume the agent with
  "machine free" and V1's `REF` pinned to this completion commit; then its
  fast check, V1's landing, Debug self-play and `gate_extra`, and V1's SPRT
  `{0, 5}`. Machine idle; no watcher armed.
...
- **S231 is done, 2026-09-20, on a measured zero, and `plan_current/` is
  ...
  **DEC-224** records why the idea leaves: the census had the table read
  ...
  one-ply table's H1 (DEC-210) stands; DEC-222's two removal verdicts are
  not owed; `ContHistWeight`'s ceiling is 2000 again. The ledger holds
  twenty-one (regenerated by `tools/ledger.py`: mean 4 h 30 m, median 4 h
  26 m, 211329 games in 94.82 h; slow class eleven at 6 h 49 m) and the
  pending count is 46 to 56 verdicts, 299 to 367 machine-hours. Two
  findings parked as filler, test-side: phase one's 17321 label for
  `3a649c0`'s node-budget count (the byte-identical tree reads 16256), and
  `S192_node_budget.py`'s drift line reading OUTSIDE for a band it just
  derived. **Machine idle, no watcher armed. Next: S095** -- its agent's
  uncompiled code is in `git stash` ("S095 in progress"); pop it, resume the
  agent with "machine free" to rebase onto the reverted tree (golden count
  50 -> 51, X-macro beside S098's four terms, `ContHist2*` gone), build,
...
- **S231's verdict is H0, 2026-09-20 10:23:54: `Elo -2.65 +/- 4.82`, `nElo
  ...
  hour, 0 forfeits, `Ptnml [556, 1464, 2097, 1352, 566]`, LOS 14.08 %.** The
  pre-registered H0 reading binds: `src/` returns to `3a649c0` whole (the
  one-ply three back to S222's fitted 17 / 18 / 26, nothing stranded), the
  two-ply idea leaves the plan with a decision, the two DEC-222 removal
  verdicts are not owed. The nElo interval [-9.60, +2.80] has its centre
  ...
  with the S192 node-budget golden re-derived on the reverted tree; then
  S231's completion (DEC-224, stamp, `plan_done/`, ledger regenerated by
  `tools/ledger.py`, specs amended); then S095's stash
  (`git stash list`, "S095 in progress") re-applied and its agent resumed
  with "machine free" to rebase, build and verify. Machine idle; no watcher
  armed.
...
- **S210 is done, 2026-09-14 10:30: F22's `--nonreg` SPRT is H1, `Elo +0.01
  ...
  truth sat on the bound, so the run walked the full 25591-game expectation
  and a little past it, at 2155 an hour, 0 forfeits, crash census quiet,
  pair variance 0.3108. **The change-specific reading**: 1661 games ended by
  insufficient material and 0 carry a mate score -- the reader's first pass
  mapped the log's game numbers to PGN rounds as if games and named one
  wrong hit; corrected (rounds are pairs, `(N+1)//2`, the White name picks
  the game) before anything was banked. Ten defects closed over the two
  halves (DEC-208; the stamp has the list); `Bench` 7105111; the ledger
  holds thirteen verdicts. `specs.md`'s F22 passage carries the figure.
  Evidence `adocs/data/S210_f22_sprt.log`, `S210_f22_sprt_pairs.txt`.
  `plan_current/` empty after this commit; machine idle. **Next: S091**,
  capture SEE pruning and the extra reduction of a negative-SEE move, one
  SPRT, brief ready and reconciled with S109. **Compaction point.**
...
- **S210's second half landed, 2026-09-13 17:05, by an Opus 5 subagent
  ...
  `quiescence()`'s loop, unconditional so a quiet promotion in check is
  covered -- red first (`CHECK( 65 == 0 )`, 361, 407 on KvK, KNvK, KBvK
  with 24 preconditions), the over-firing mutant killed by the check
  stand-pat case. **The census** (`adocs/data/S210_f22_census.py`, 11503
  positions of the S219 A/A at depth 10): 0.062 % of quiescence moves land
  on a dead board, **194 of 11503 root answers move (1.687 %)**, 444 root
  scores; DEC-107's discharge (S162 read 0 in 3314 games) does not reach,
  and the Debug self-play saw 7 of 80 games end by insufficient material.
  **One `--nonreg` `{-5, 0}` SPRT is owed**, pre-registered in
  `adocs/data/S210_f22_sprt.sh` against `9075bf8`: 25591 games / 12.1 h if
  the truth sits on the bound, 41861 / 19.8 h at the midpoint -- a night run
  (DEC-155); the coordinator pins `CAND` to F22's commit after committing it
  so the day's neutral steps cannot enter the candidate, and launches it
  tonight. **`Bench: 7105111`** (from 7111579); the six `search_bench` rows
  identical, which says only that those positions never reach the class;
  Debug self-play 88 games 0 `Assertion`; gate 38/38 both builds.
  `MANUAL.md` checked, no change; `DEV_MANUAL.md`'s signature history gains
  the S210 line; `specs.md`'s search row carries the passage. First half in
  `9075bf8`; `gate_extra.sh` runs before the stamp (the search was touched).
  **The fast check found nothing wrong in the fix** and five trivial items,
  taken by the coordinator: the dead score written in the child's frame,
