...
| `test_search_params.cpp` `golden_defaults` | **63** defaults and their ranges — 50 until S095 added `LmrNoTtMove`, then S097's **six** singular-extension rows (four settings and the two switches `SeExtend` and `SeMultiCut`) and S132's three node-fraction rows (`TmNodeScalePct`, `TmNodeBasePct`, `TmNodeMinDepth`). S188 added three and its H0 took them out again; S236 added three and its two verdicts took **two** of them out again — `LmrHistDiv` and `LmrHistClamp` left with the history term (DEC-231) and `LmrRoundBias` stayed with the accumulator, which is why the count went to 61 and not 60. Then S234's `RfpTtEstimate` and S235's `RfpReturnWeight`, one each, which is 63; this row read 61 against a table of 62 between S234's landing and S235, and the two moves are stated together here rather than the second one silently repairing the first | no script: `src/search_params.hpp` is the derivation |
...
**At `S188`, the check extension: `5756104`, +28.1 % -- and then `4493659`
again, because the rule left the tree on its own verdict.** The ledger keeps
both halves because the pair is the lesson. Three forms were measured before a
game was played, against the bar DEC-228 stated -- at most one ply lost on any
of the three `search_bench` positions at `go nodes 1000000` and at most two in
total, `bench` under +30 %, every fast-suite case inside its ceiling:
...
The third met every clause and was the one the SPRT measured: **H0, `Elo
-13.84 +/- 8.95`, `nElo -17.71 +/- 11.44` over 3542 games**, the interval
wholly below zero, so the rule was removed in the same step and `bench` is
`4493659` again -- `d819a09`'s own total, with all eight `bestmove` replies
identical to a build of that commit and `tools/search_bench.py` identical at
depths 9 and 12, which is INV-6's discharge of a removal. **What the
instrument could and could not see**: the fixed-node depths said the third
form cost one ply where the first cost five, and the S145 mined set said it
found 180 mates at depth 10 against 146 -- the largest move that instrument
has recorded -- and the games priced both at -13.84 Elo. A fixed-depth mate
count is a reading of a larger tree and not of a better one (DEC-019), and
this pair is the sharpest example of that the ledger holds.
...
**At `S235`, the reverse-futility return blended toward beta: `4803214` ->
`4823539`, +0.42 %.** The rule that decides is untouched
and the nodes that prune are the same nodes -- what moves is the number a pruned
node hands its parent: `beta + (bound - beta) * RfpReturnWeight / 100`, where
`bound` is what the site's own test argued and the weight ships at the declared
range's midpoint, 50. The total moves because a smaller fail-soft bound is a
weaker claim: the parent's own best score, the entry it stores and the
aspiration window above it all see a different number, so the tree below and
beside the pruned node reorders. Nothing about the direction of that move is a
prediction (DEC-019) and `adocs/data/S235_sprt.sh` is what prices it.

**The off value is proved on the tree and not declared from a range's end**
(DEC-215): a Release build with `RfpReturnWeight` defaulted to 100 returns
`bound` itself and must print the reference commit's own total with all eight
`bestmove` replies identical, with `tools/search_bench.py` reproducing that
commit at depths 9 and 12. MEASURED 2026-09-25: **`4803214`**, `169b4cb`'s own
total to the node, with all eight replies identical -- c3d5 d5e6 d7c8q g7h8q
d8e7 a1b2 e5e6 e5e6 -- and `search_bench` reproducing that commit at both
depths, 48522 / 85714 / 28080 at 9 and 129499 / 411457 / 172984 at 12, every
best move the parent's.
