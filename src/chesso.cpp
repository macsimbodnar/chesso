...
// Share of the move's time budget after which no new iteration is started. The
// remainder is left to the rest of the game rather than spent on an iteration
...
uci_search_result_t iterative_deepening_search(const uci_search_options_t& conf)
{
  ...
  // An iteration that runs out of time is no longer wasted - its partial root
  // result is kept - so there is nothing to predict about the next one. The
  // cutoff is simply the point past which the leftover time is worth more to
  // the moves that follow. An explicit `movetime` is not a shared budget, so it
  // is spent in full.
  ...
  for (int current_depth = 1; current_depth <= conf.depth; ++current_depth) {
    ...
    // An aborted iteration is still worth harvesting. The root replaces its
