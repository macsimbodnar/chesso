...
int evaluate(const board_t* board)
{
  ...
  for (int pc = W_PAWN; pc <= B_KING; ++pc) {
    bb_t current_board = board->bitboards[pc];

    while (current_board) {
      const index_t index = get_lsb_index(current_board);

      if (pc != EMPTY) {
        mg[PCOLOR(pc)] += mg_table[pc][index];
        eg[PCOLOR(pc)] += eg_table[pc][index];
  ...
  /* tapered eval */
  int mgScore = mg[board->active_color] - mg[OTHER(board->active_color)];
  int egScore = eg[board->active_color] - eg[OTHER(board->active_color)];
