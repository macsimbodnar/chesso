...
| `LmrShallowerMargin` | 47 | 0 to 94 | how narrowly a reduced move's zero-window score may beat alpha for the re-search to be run **one ply shallower** instead. Same scale. **0 switches this path off**, by arithmetic: the re-search happens only where the score is above alpha. Where both this and `LmrDeeperMargin` would fire, this one wins |
