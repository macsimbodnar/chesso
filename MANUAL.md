...
| `CheckExtend` | 1 | 0 to 1 | whether a move that gives check is searched one ply deeper. At 1 a checking move is, **provided static exchange at threshold zero holds for the move itself** — the exchange on the square the moved piece lands on, so a move that gives check and hangs the piece it moved is not extended; on a discovered check the piece weighed is the one that moved and not the one giving check — and provided the node is not the root, its ply has not reached `CheckExtPlyFactor * depth` and its remaining depth is at most `CheckExtMaxDepth`. Captures and quiets alike. The ply is shared with the singular extension above, so a move both rules want is searched one ply deeper and not two. At **0** no move is extended for giving check and the engine is the one before S188, to the node — which is the off value neither setting below has inside its range. It is a verdict switch and not something to tune |
| `CheckExtPlyFactor` | 4 | 1 to 7 | the explosion cap on that extension: no move is extended at a node whose ply has reached `CheckExtPlyFactor * depth`. An extended child keeps its parent's remaining depth, so along a chain of checks the depth never falls and only the ply rises; this is what stops such a chain, the hard ply wall being the only other thing that would. At 1 a line may not outgrow the remaining depth of the node that started it; at 7 the wall is close to being what binds. **No value in this range switches the extension off** |
| `CheckExtMaxDepth` | 8 | 1 to 16 | how close to the horizon that extension is allowed: no move is extended at a node whose remaining depth is above `CheckExtMaxDepth`. Extending near the root multiplies a subtree that is most of the search, where the same forcing line near the horizon costs a fraction of it. At 16 the restriction never binds at the depths this engine reaches and stops being one; **no value in this range switches the extension off** |
