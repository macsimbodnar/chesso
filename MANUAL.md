...
| `ContHistWeight` | 26 | 0 to 1000 | how much of the quiet ordering band the one-ply continuation term may span, as a percentage of its raw entry: `score_move` returns `quiet_history + ContHistWeight * cont_hist / 100 + ContHist2Weight * cont_hist2 / 100`. At the fitted setting the term spans 8519 against plain history's fitted 8831, so the two carry very nearly equal authority — the seed put them at 8191 against 8192 and this project's own SPSA lane at S222 barely moved either. The continuation table's own bound is fixed at 32767, the `int16_t` ceiling, and is not an option: a bound and a weight over one table are one degree of freedom, not two. **The range was 0 to 2000 until S231 halved it**, and the halving is arithmetic: the quiet band now carries two weighted terms, the clearance against the countermove band is a property of their total, and 1000 + 1000 spans the same 655340 that 2000 alone did — the same 11893 of clearance, and the fitted 26 untouched |
| `ContHist2Bonus` | 17 | 0 to 1000 | two-ply continuation history, S231: the bonus its cutoff cell is credited with, in the same unit as `ContHistBonus` — thousandths of that table's own band at remaining depth 11 — for the table keyed on the move **two** plies back and this move. This and the two rows below are first settings, seeded from this project's own numbers and not yet fitted: the two shares from the one-ply table's fitted 17 and 18, because the two tables are written at the same call site over the same spans into bands of the same width |
| `ContHist2Malus` | 18 | 0 to 1000 | the same unit, for the malus every quiet tried before the cutoff move at that node is charged in the two-ply table |
| `ContHist2Weight` | 26 | 0 to 1000 | how much of the quiet ordering band the two-ply term may span, on `ContHistWeight`'s definition and sharing its band-clearance ceiling. At 26 the term spans 26 × 32767 / 100 = 8519, which is what the one-ply term spans at its own fitted 26, so the quiet band ships as 8831 / 8519 / 8519 over its three terms and the two tables start with equal authority. 0 switches the table off — nothing is read and the ordering is the one-ply engine's exactly — and it is deliberately not the default: the lane that fits this weight cannot start it at a bound. S231's own SPSA lane (`tools/spsa_s231.json`) fits all six continuation axes before the SPRT that decides the step |
