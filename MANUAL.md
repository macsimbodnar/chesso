...
| `SeMultiCut` | 0 | 0 to 1 | whether the same verification search may also **return**: at 1, a verification that fails high at or above this node's own beta ends the node with the score it found, never at a PV node and never with a mate-range value. It is the second half of S097 and ships off, so the engine as released runs the extension alone; the two are separate changes with separate measurements and this is the switch between them |
