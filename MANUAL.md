...
| `ContHistBonus` | 15 | 0 to 1000 | one-ply continuation history: the bonus its cutoff cell is credited with, in **thousandths of that table's own band at remaining depth 11**, graded quadratically in the remaining depth like plain history's. 0 switches the table off — nothing is written and the ordering term is identically 0 — and 1000 is one whole band in a single update at that depth, past which the clamp makes every larger value the same engine |
| `ContHistMalus` | 15 | 0 to 1000 | the same unit, for the malus every quiet tried before the cutoff move at that node is charged in the continuation table |
| `ContHistWeight` | 25 | 0 to 2000 | how much of the quiet ordering band the continuation term may span, as a percentage of its raw entry: `score_move` returns `quiet_history + ContHistWeight * cont_hist / 100`. At 25 the term spans 8191 against plain history's shipped 8192, so the two start with equal authority. The continuation table's own bound is fixed at 32767, the `int16_t` ceiling, and is not an option: a bound and a weight over one table are one degree of freedom, not two. 2000 is where the widened quiet band would stop clearing the countermove band by 100 |
