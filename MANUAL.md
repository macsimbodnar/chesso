...
| `LmrHistDiv` | 699 | 1 to 34700 | how much raw history buys one ply back: the reduction is `table - clamp(history / LmrHistDiv, +/-LmrHistClamp)`, where the history is the butterfly entry plus `ContHistWeight` per cent of the continuation entry — the same sum `score_move` orders a plain quiet by, and never a killer's or a countermove's band. This project's own SPSA fit at S098, 60000 games at 2+0.02, from a seed of 430 — the **75th percentile of that sum's magnitude where the rule reads it**, measured over 400 self-play positions at depth 12 (`adocs/data/S098_v1_hist_census.txt`). The floor is arithmetic; the range top is twice the sum's own saturation, 2 × 17350, and switches the term off by making every quotient zero |
