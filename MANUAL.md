...
| `ContHistBonus` | 18 | 0 to 1000 | one-ply continuation history: the bonus its cutoff cell is credited with, in **thousandths of that table's own band at remaining depth 11**, graded quadratically in the remaining depth like plain history's. 0 switches the table off — nothing is written and the ordering term is identically 0 — and 1000 is one whole band in a single update at that depth, past which the clamp makes every larger value the same engine. This and the two rows below carry this project's own SPSA fit at S231 (2026-09-19/20, six continuation axes together, 60000 games), which moved them 17 → 18, 18 → 17 and 26 → 24. What that lane started from was S222's fit of the same three, 15 → 17, 15 → 18 and 25 → 26, and those are seeds now and not values in force |
| `ContHistMalus` | 17 | 0 to 1000 | the same unit, for the malus every quiet tried before the cutoff move at that node is charged in the continuation table |
| `ContHistWeight` | 24 | 0 to 1000 | how much of the quiet ordering band the one-ply continuation term may span, as a percentage of its raw entry: `score_move` returns `quiet_history + ContHistWeight * cont_hist / 100 + ContHist2Weight * cont_hist2 / 100`. At the fitted setting the term spans 24 × 32767 / 100 = 7864, against plain history's fitted 8831 and the two-ply term's own 7864, so the three carry very nearly equal authority — the first settings put the one-ply term at 8191 against plain history's 8192, and two of this project's own SPSA lanes have moved it since, to 8519 at S222 and 7864 at S231. Either continuation table's own bound is fixed at 32767, the `int16_t` ceiling, and is not an option: a bound and a weight over one table are one degree of freedom, not two. **The range was 0 to 2000 until S231 halved it**, and the halving is arithmetic: the quiet band now carries two weighted terms, the clearance against the countermove band is a property of their total, and 1000 + 1000 spans the same 655340 that 2000 alone did — the same 11893 of clearance, and it moved no value of its own |
| `ContHist2Bonus` | 18 | 0 to 1000 | two-ply continuation history, S231: the bonus its cutoff cell is credited with, in the same unit as `ContHistBonus` — thousandths of that table's own band at remaining depth 11 — for the table keyed on the move **two** plies back and this move. This and the two rows below were seeded from this project's own numbers, the two shares from the one-ply table's then fitted 17 and 18 because the two tables are written at the same call site over the same spans into bands of the same width, and S231's own SPSA lane has since fitted all three: 17 → 18, 18 → 20 and 26 → 24 |
| `ContHist2Malus` | 20 | 0 to 1000 | the same unit, for the malus every quiet tried before the cutoff move at that node is charged in the two-ply table |
| `ContHist2Weight` | 24 | 0 to 1000 | how much of the quiet ordering band the two-ply term may span, on `ContHistWeight`'s definition and sharing its band-clearance ceiling. At 24 the term spans 24 × 32767 / 100 = 7864, which is exactly what the one-ply term spans at its own fitted 24, so the quiet band ships as 8831 / 7864 / 7864 over its three terms and the two tables carry equal authority — which is where the seed put them and where the fit left them, two units lower on both. 0 switches the table off — nothing is read and the ordering is the one-ply engine's exactly — and it was deliberately not the seed: a lane whose subject is this weight cannot start it at a bound. The fit is S231's own lane, `tools/spsa_s231.json` and `adocs/data/S231_spsa_trajectory.tsv`; one gainer SPRT against the tree before the table decides the step |
