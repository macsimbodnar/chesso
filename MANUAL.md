...
| `LmrHistClamp` | 2048 | 0 to 2048 | how much of a ply that history term may ever be worth, in the same ticks: 1024 is one ply, so the default of 2048 is two of them — **the top of the range, which the census's seed rule overshot** (it asked for 6.7 plies from a tail four times longer than the one the rule was written for), so this default is a bound and not a fit. **0 switches the term off** and at 0 the reduction is the table and the node terms alone. The top is two plies, past which the term would replace the ordering's estimate rather than adjust it |
