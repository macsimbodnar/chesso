...
| `LmrHistDiv` | 1442 | 1 to 34700 | how much raw history buys one ply back: the reduction is `table - clamp(history / LmrHistDiv, +/-LmrHistClamp)`, where the history is the butterfly entry plus `ContHistWeight` per cent of the continuation entry — the same sum `score_move` orders a plain quiet by, and never a killer's or a countermove's band. The **90th percentile of that sum's magnitude where the rule reads it**, over 400 self-play positions at depth 12 (`adocs/data/S098_v1_hist_census.txt`), so the term reaches a ply for a tenth of the moves it sees. It is S098 verdict 1's second bisection leg and not a fit: the SPSA fit of 699 measured H0 at −3.70 nElo over 11524 games and `adocs/data/S098_v1_leg2_sprt.sh` prices this value. The floor is arithmetic; the range top is twice the sum's own saturation, 2 × 17350, and switches the term off by making every quotient zero |
| `LmrHistClamp` | 3 | 0 to 4 | the most plies that term may move a reduction, in either direction. **0 switches it off** and the reduction is the table's again — the value a bisection rebuild uses. This project's own SPSA fit at S098 in the same lane as the row above, from a seed of 2, which was half of what the table returns at the median depth and the last move index (`0.52 + ln(11) * ln(63) / 1.82` = 5.98 floored to 5); at 4 the term would exceed what the table returns for the first reducible move at every depth this engine reaches, which is a different rule and not a setting of this one |
