...
| `RfpTtEstimate` | 1 | 0 to 1 | which number the reverse-futility margin is subtracted from. At **1**, the value since S234, it is the node's *estimate*: the table's own score for this position where the entry's bound certifies the direction that score has moved in from the static evaluation — a lower bound above it may raise the number, an upper bound below it may lower it, and nothing else is taken, never a score inside the mate band and never at a node in check. The bound the rule returns is that same number less the margin. At **0** both read the static evaluation and the engine is the one before S234. What is stored in the table's evaluation field, and what the improving flag compares across plies, is the raw static evaluation at either setting. The same estimate has fed the futility margin in the move loop since S109 and is not switched there |
| `RfpReturnWeight` | 50 | 0 to 100 | what a node pruned by reverse futility hands back, in **hundredths of the gap** between beta and the bound that rule's own test argued for: the value returned is `beta + (bound - beta) * RfpReturnWeight / 100`, computed at the return of the reverse-futility block in `negamax_at`, `src/search.cpp`. At **100** it is that bound itself, which is the engine before S235 and is this parameter's off value; at **0** it is beta exactly, which is still a legal fail-soft bound. The division floors, so where the share cannot be paid exactly the node returns the point below it, toward beta. It moves what a pruned node returns and never whether the node prunes, and it is independent of `RfpTtEstimate` above — it blends whichever number that row leaves the margin to be subtracted from. 50 is the declared range's midpoint and not a fitted value; S127 fits it |
