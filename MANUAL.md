...
| `LmrDeeperMargin` | 47 | 0 to 94 | how far a reduced move's zero-window score must clear the node's own best score so far before the re-search is run **one ply deeper** than the move would otherwise get. In chesso's material scale, so 94 is a pawn and the range's top is the point past which the decision would rest on more material than a pawn of window. The range top is **not** an off value for this path: the best score sits below alpha at every scout node, so the condition still fires there |
| `LmrShallowerMargin` | 0 | 0 to 94 | how narrowly a reduced move's zero-window score may beat alpha for the re-search to be run **one ply shallower** instead. Same scale. **It ships at 0, which switches this path off**, by arithmetic: the re-search happens only where the score is above alpha. That is leg 1 of the bisection S098 verdict 3's H0 opened, and the range keeps its 47 for the leg that would put it back. Where both this and `LmrDeeperMargin` would fire, this one wins |
| `LmrDeeperMinReduction` | 2 | 1 to 126 | the smallest reduction the deeper path will act on. 1 is the bare form, which the site already guarantees and which the published record measured negative; **126 switches the deeper path off**, being above every reduction the clamp to `[0, child_depth - 1]` admits. The shallower path carries a fixed guard of 2 instead, which is arithmetic and not a setting: at a reduction of 1 a shallower re-search is the reduced search repeated |
