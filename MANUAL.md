...
|---|---|---|---|
| `QuietHistoryMax` | 8831 | 1 to 32767 | the gravity bound on a quiet history entry. Every score the table holds lies in `[-QuietHistoryMax, +QuietHistoryMax]`, which is the whole quiet ordering band, both edges. Both bounds are arithmetic: the update divides by this value, and the entry is an `int16_t`. Replaced `OrderHistoryMax` at S093, whose 600000 was a saturation ceiling on an unbounded accumulator. Fitted by this project's own SPSA lane at S222 from the 8192 it shipped as |
| `HistoryBonusQuad` | 6 | 0 to 1024 | the `depth * depth` coefficient of the bonus a quiet move that caused a cutoff is credited with |
| `HistoryBonusLin` | 19 | 0 to 4096 | the `depth` coefficient of the same bonus |
| `HistoryBonusConst` | 2 | -32768 to 32767 | the constant term of the same bonus. Negative is legal: the published linear form is `300 * depth - 250` |
| `HistoryMalusQuad` | 0 | 0 to 1024 | the `depth * depth` coefficient of the malus every quiet tried before the cutoff move at that node is charged |
| `HistoryMalusLin` | 17 | 0 to 4096 | the `depth` coefficient of the same malus |
| `HistoryMalusConst` | 36 | -32768 to 32767 | the constant term of the same malus. All six coefficients are this project's own SPSA fit at S222, the first that ever moved them off the `depth * depth` seed they shipped as; the fit does split the two halves, leaving the bonus quadratic and the malus linear |
| `ContHistBonus` | 17 | 0 to 1000 | one-ply continuation history: the bonus its cutoff cell is credited with, in **thousandths of that table's own band at remaining depth 11**, graded quadratically in the remaining depth like plain history's. 0 switches the table off — nothing is written and the ordering term is identically 0 — and 1000 is one whole band in a single update at that depth, past which the clamp makes every larger value the same engine. This and the two rows below are this project's own SPSA fit at S222, from seeds of 15, 15 and 25 |
| `ContHistMalus` | 18 | 0 to 1000 | the same unit, for the malus every quiet tried before the cutoff move at that node is charged in the continuation table |
| `ContHistWeight` | 26 | 0 to 2000 | how much of the quiet ordering band the continuation term may span, as a percentage of its raw entry: `score_move` returns `quiet_history + ContHistWeight * cont_hist / 100`. At the fitted setting the term spans 8519 against plain history's fitted 8831, so the two carry very nearly equal authority — the seed put them at 8191 against 8192 and this project's own SPSA lane at S222 barely moved either. The continuation table's own bound is fixed at 32767, the `int16_t` ceiling, and is not an option: a bound and a weight over one table are one degree of freedom, not two. 2000 is where the widened quiet band would stop clearing the countermove band by 100 |
...
| `LmrRoundBias` | 0 | 0 to 1023 | the reduction's rounding rule. The reduction is summed in **ticks of a ply** — 1024 of them to the ply, a compile-time constant beside `build_lmr_table` in `src/search.cpp` — and turned into whole plies once, as `(ticks + LmrRoundBias) >> 10`, so this value is the rule and not a coefficient of it: **0 floors**, which is what the engine did before this parameter existed, and it is both the off value and the shipped default: at 512 — round-to-nearest, the value the range's midpoint asks for — the engine loses a mate the fast suite guards, and so does every bias from 1 tick upward with the history term live. 1023 rounds up. At `LmrRoundBias` 0 the reduction the engine takes is the one it took before S236, so the only change this parameter's step makes to play is the history term below |
| `LmrHistDiv` | 734 | 1 to 35532800 | the history sum that buys one whole ply of **less** reduction on a late quiet: the term is `hist_sum * 1024 / LmrHistDiv` ticks, clamped, subtracted from the reduction, so a quiet the history tables like is searched less shallowly and one they have written off more so. The sum is the raw butterfly entry plus the weighted continuation entry — the same **expression** the move ordering scores a quiet with, evaluated again here rather than carried: the ordering fills its scores once when the moves are generated, and by the time a late quiet reaches the reduction the plies searched in between may have written to both tables, so the two readings are the same rule over possibly different entries. Seeded from a census of that sum at the reduction's own sites (`adocs/data/S236_hist_census.txt`) so the ninetieth percentile site is worth half a ply; more than two thirds of the sites have a sum of zero and are not moved at all. The range top is twice the saturated sum in ticks, at which no sum the tables can hold reaches one tick |
| `LmrHistClamp` | 1024 | 0 to 2048 | how much of a ply that history term may ever be worth, in the same ticks: **1024 is exactly one ply**. The census's seed rule asked for 6.7 plies and the range top bound it to two, which is what S236 measured; that read as a walk — nElo −2.05 ± 5.44 over 15658 games, an interval reaching above zero — and its **one pre-registered follow-up** is this default, the same term at half the reach. The census said before any game that the same 12.54 % of reduction sites move either way and only the price changes: +15.6 % of tree here against +52.6 % at two plies. **0 switches the term off** and at 0 the reduction is the table and the node terms alone. The top is two plies, past which the term would replace the ordering's estimate rather than adjust it |
| `LmrCutNode` | 1 | 0 to 2 | plies added to the reduction at a node the search predicts will fail high. The prediction is the published node-type alternation -- the root is a PV node, the first child of a PV node is a PV node and every later one is scouted as a CUT node, the first child of a CUT node is an ALL node, children of ALL nodes are CUT nodes. 0 switches the term off |
| `LmrNotImproving` | 1 | 0 to 2 | plies added where the side to move is **not** better off than it was two plies ago. The asymmetry is deliberate: there is no term for improving, and reducing less when improving is the direction that failed where it was tried. 0 switches it off |
| `LmrTtCapture` | 1 | 0 to 2 | plies added where the transposition-table entry's own move is a capture, which says the node is tactical and the quiet being reduced is not what it is about. The move under the table is quiet by that rule's own eligibility. 0 switches it off |
| `LmrPv` | 1 | 0 to 2 | plies **subtracted** at a principal variation node, whose line gets reported and played. The only negative term of the four, and the only one the shallow-depth pruning gate never sees -- a PV node is not a pruning node. 0 switches it off |
| `LmrNoTtMove` | 1 | 0 to 2 | plies added where the transposition-table entry carries no move at all -- either there is no entry, or there is one that quiescence wrote without a move, which since S094 is the only way a moveless entry is made. Nothing has searched this node properly yet, so it is cheaper than its depth claims and its late quiets are reduced one ply more. 0 switches it off, and at 0 the engine is the one before S095 |
| `LmrDeeperMargin` | 47 | 0 to 94 | how far a reduced move's zero-window score must clear the node's own best score so far before the re-search is run **one ply deeper** than the move would otherwise get; everywhere else the re-search runs at the depth the move would have got anyway. In chesso's material scale, so 94 is a pawn and the range's top is the point past which the decision would rest on more material than a pawn of window. The range top is **not** an off value: the best score sits below alpha at every scout node, so the condition still fires there. A second path that ran the re-search one ply *shallower* shipped beside this one until 2026-09-18 and was removed after an SPRT bisection attributed a measured loss to it |
| `LmrDeeperMinReduction` | 2 | 1 to 126 | the smallest reduction the deeper re-search will act on. 1 is the bare form, which the site already guarantees and which the published record measured negative; **126 switches the whole rule off**, being above every reduction the clamp to `[0, child_depth - 1]` admits, and the engine is then the one before S098 verdict 3 |
...
| `HistPruneCoeff` | 612 | 0 to 16384 | history pruning: a quiet whose butterfly history is below `-HistPruneCoeff * lmr_depth` is skipped. This project's own SPSA fit at S222, from the 576 it shipped as, which was the midpoint of the region `QuietHistoryMax/64` to `QuietHistoryMax/8` |
...
| `SeExtend` | 1 | 0 to 1 | whether the singular-extension block runs at all. At **0** no verification search is made and no move is extended, and the engine is the one before S097 — the off value the four settings below do not have between them, because `SeMinDepth` at its own range top still fires at every depth above 16. It is a verdict switch and not something to tune |
| `SeMinDepth` | 10 | 4 to 16 | the shallowest remaining depth a singular extension is looked for at. The node's table move is set aside and the node searched again at about half depth against a window below the entry's score; if every other move fails under that window the table move is searched one ply deeper. The floor is where the halved search still keeps a real ply; the top is where the rule reaches only the last iterations of a search. **No value in this range switches the feature off** — at 16 it still fires at every depth above 16 |
| `SeTtDepthMargin` | 4 | 0 to 8 | how far below this node's remaining depth the table entry may have been searched and still be trusted to name the move worth verifying: the rule wants `entry depth >= depth - SeTtDepthMargin`. At 0 only an entry as deep as this search counts |
| `SePlyFactor` | 5 | 2 to 8 | the explosion cap on the extension: no verification is run at a node whose ply is at or above `SePlyFactor * depth`. An extension adds depth, so a line that keeps extending is bounded by this first and by the hard ply wall second |
| `SeMarginPerDepth` | 9 | 1 to 18 | how far below the entry's score the verification window sits, per remaining ply: `singular beta = entry score - SeMarginPerDepth * depth`. In chesso's material scale, where a pawn is 94, so 18 is the value at which the margin reaches two pawns at the shipped `SeMinDepth`. **No off value is in this range** and the verification depth, `(depth - 1) / 2`, is not an option at all |
| `SeMultiCut` | 1 | 0 to 1 | whether the same verification search may also **return**. At 1, the value since S097's second verdict, a verification that fails high at or above this node's own beta ends the node there and hands back **the score that search found** — never `singular beta` itself, never at a PV node, never a value inside the mate band, and never against a beta inside the negative mate band. The node searches no move of its own, so this is pruning and not ordering. At **0** the branch goes and nothing else does: the verification still runs and still extends, and the engine is the one S097's first verdict landed. The two halves of S097 are separate changes with separate measurements and this is the switch between them |
